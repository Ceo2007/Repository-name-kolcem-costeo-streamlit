...
def _mask_diferencia_cambio(df_in: pd.DataFrame) -> pd.Series:
    """Detecta filas de diferencia en cambio sin depender del nombre exacto del auxiliar.

    Regla gerencial: diferencia en cambio realizada puede ser financiero recurrente;
    diferencia en cambio no realizada se separa del costo recurrente y se lleva al
    resultado final contable.
    """
    if df_in is None or df_in.empty:
        return pd.Series(False, index=df_in.index if df_in is not None else [])
    obs = df_in["Obs_norm"].fillna("")
    idx = df_in["Indice_norm"].fillna("")
    mask_obs = (obs.str.contains("DIFERENCIA", na=False) & obs.str.contains("CAMBIO", na=False)) | obs.str.contains("DIF CAMBIO", na=False)
    mask_idx = (idx.str.contains("DIFERENCIA", na=False) & idx.str.contains("CAMBIO", na=False)) | idx.str.contains("DIF CAMBIO", na=False)
    return mask_obs | mask_idx


def _mask_diferencia_cambio_no_realizada(df_in: pd.DataFrame) -> pd.Series:
    if df_in is None or df_in.empty:
        return pd.Series(False, index=df_in.index if df_in is not None else [])
    obs = df_in["Obs_norm"].fillna("")
    idx = df_in["Indice_norm"].fillna("")
    mask_no_realizada = obs.str.contains("NO REALIZ", na=False) | idx.str.contains("NO REALIZ", na=False)
    return _mask_diferencia_cambio(df_in) & mask_no_realizada


def _filtrar_indices_opcional(df_in: pd.DataFrame, indices: Optional[Iterable[str]] = None) -> pd.DataFrame:
    if df_in is None or df_in.empty:
...
def suma_diferencia_cambio_no_realizada(df_in: pd.DataFrame, indices: Optional[Iterable[str]] = None) -> float:
    base = _filtrar_indices_opcional(df_in, indices)
    if base.empty:
        return 0.0
    return float(base.loc[_mask_diferencia_cambio_no_realizada(base), "Valor"].sum())


def suma_diferencia_cambio_realizada(df_in: pd.DataFrame, indices: Optional[Iterable[str]] = None) -> float:
    base = _filtrar_indices_opcional(df_in, indices)
    if base.empty:
        return 0.0
    mask_fx = _mask_diferencia_cambio(base)
    mask_no_real = _mask_diferencia_cambio_no_realizada(base)
    return float(base.loc[mask_fx & ~mask_no_real, "Valor"].sum())


def excluir_diferencia_cambio_no_realizada(df_in: pd.DataFrame) -> pd.DataFrame:
    """Quita el efecto no realizado del costo recurrente sin eliminarlo del reporte."""
    if df_in is None or df_in.empty:
        return pd.DataFrame(columns=df_in.columns if df_in is not None else [])
    return df_in.loc[~_mask_diferencia_cambio_no_realizada(df_in)].copy()
...
def detalle_gastos_comerciales(df_in: pd.DataFrame) -> pd.DataFrame:
    ...
    if _producto_actual_es_art():
        ...
        base = excluir_diferencia_cambio_no_realizada(base)
        return base
    idx = expandir_indices(INDICES_GASTOS_COMERCIALES)
    base = df_in[df_in["Indice_norm"].isin(idx)].copy()
    return excluir_diferencia_cambio_no_realizada(base)
...
def suma_gastos_comerciales(df_in: pd.DataFrame) -> float:
    ...
    if _producto_actual_es_art():
        ...
        total -= suma_diferencia_cambio_no_realizada(df_in)
        return float(total)
    total = suma_indices(df_in, INDICES_GASTOS_COMERCIALES)
    total -= suma_diferencia_cambio_no_realizada(df_in, INDICES_GASTOS_COMERCIALES)
    return float(total)
...
def suma_financiera_comercial(df_in: pd.DataFrame) -> float:
    ...
    if _producto_actual_es_art():
        ...
        fin_total = fin_subtotal + suma_indices(df_in, INDICES_FINANCIEROS)
        fin_total -= suma_diferencia_cambio_no_realizada(df_in, INDICES_FINANCIEROS)
        return fin_total
    fin_total = suma_indices(df_in, INDICES_FINANCIEROS)
    fin_total -= suma_diferencia_cambio_no_realizada(df_in, INDICES_FINANCIEROS)
    return fin_total
...
# ------------------------------------------------------------
# Conciliación contable opcional · P&G vs costeo gerencial
# ------------------------------------------------------------

@st.cache_data(show_spinner=False)
def load_contabilidad(uploaded_bytes: bytes) -> pd.DataFrame:
    """Lee una contabilidad auxiliar tipo Hoja2 con Ano, Mes, Auxiliar, NombreAux, TotalDebito, TotalCredito, Saldo.

    Regla dura CFO: la conciliación P&G usa cuentas 4/5/6. Las clases 1/2/3 se muestran solo como control,
    nunca como gasto. Esto evita que provisiones de balance contaminen la utilidad gerencial.
    """
    xls = pd.ExcelFile(io.BytesIO(uploaded_bytes), engine="openpyxl")
    candidatos: list[pd.DataFrame] = []
    for sh in xls.sheet_names:
        # Lectura normal primero
        try:
            df0 = pd.read_excel(xls, sheet_name=sh)
            candidatos.append(df0)
        except Exception:
            pass
        # Lectura sin encabezado para detectar tablas pegadas con encabezados internos
        try:
            raw = pd.read_excel(xls, sheet_name=sh, header=None)
            for i in range(min(len(raw), 25)):
                vals = [norm_text(x) for x in list(raw.iloc[i].values)]
                if "ANO" in vals and "MES" in vals and "AUXILIAR" in vals:
                    df1 = raw.iloc[i + 1:].copy()
                    df1.columns = list(raw.iloc[i].values)
                    candidatos.append(df1)
                    break
        except Exception:
            pass

    required_norm = {"ANO", "MES", "AUXILIAR"}
    for cand in candidatos:
        if cand is None or cand.empty:
            continue
        col_map = {norm_text(c): c for c in cand.columns}
        if not required_norm.issubset(set(col_map.keys())):
            continue
        out = cand.rename(columns={
            col_map.get("ANO"): "Ano",
            col_map.get("MES"): "Mes",
            col_map.get("AUXILIAR"): "Auxiliar",
            col_map.get("NOMBREAUX", col_map.get("NOMBRE AUX", col_map.get("NOMBRE"))): "NombreAux",
            col_map.get("TOTALDEBITO", col_map.get("TOTAL DEBITO", col_map.get("DEBITO"))): "TotalDebito",
            col_map.get("TOTALCREDITO", col_map.get("TOTAL CREDITO", col_map.get("CREDITO"))): "TotalCredito",
            col_map.get("SALDO"): "Saldo",
        }).copy()
        for c in ["Ano", "Mes", "Auxiliar"]:
            if c not in out.columns:
                continue
        if "NombreAux" not in out.columns:
            out["NombreAux"] = ""
        if "TotalDebito" not in out.columns:
            out["TotalDebito"] = 0.0
        if "TotalCredito" not in out.columns:
            out["TotalCredito"] = 0.0
        if "Saldo" not in out.columns:
            out["Saldo"] = out["TotalDebito"].apply(parse_valor) - out["TotalCredito"].apply(parse_valor)

        out = out[["Ano", "Mes", "Auxiliar", "NombreAux", "TotalDebito", "TotalCredito", "Saldo"]].dropna(how="all").copy()
        out["Ano"] = pd.to_numeric(out["Ano"], errors="coerce").astype("Int64")
        out["Mes"] = pd.to_numeric(out["Mes"], errors="coerce").astype("Int64")
        out["Auxiliar"] = out["Auxiliar"].apply(lambda x: str(x).replace(".0", "").strip())
        out["Auxiliar"] = out["Auxiliar"].str.extract(r"(\d+)", expand=False).fillna("")
        out["NombreAux"] = out["NombreAux"].fillna("").astype(str).str.strip()
        for c in ["TotalDebito", "TotalCredito", "Saldo"]:
            out[c] = out[c].apply(parse_valor).fillna(0.0).astype(float)
        out = out.dropna(subset=["Ano", "Mes"]).copy()
        out["Ano"] = out["Ano"].astype(int)
        out["Mes"] = out["Mes"].astype(int)
        out["Clase"] = out["Auxiliar"].str[0]
        out["Cuenta4"] = out["Auxiliar"].str[:4]
        out["Cuenta2"] = out["Auxiliar"].str[:2]
        out["Nombre_norm"] = out["NombreAux"].map(norm_text)
        return out
    raise ValueError("No encontré una tabla contable con columnas Ano, Mes, Auxiliar, TotalDebito y TotalCredito.")


def _movimiento_pyg(row: pd.Series) -> float:
    """Devuelve movimiento con signo económico P&G: ingresos positivos; costos/gastos negativos."""
    aux = str(row.get("Auxiliar", ""))
    deb = float(row.get("TotalDebito", 0.0) or 0.0)
    cre = float(row.get("TotalCredito", 0.0) or 0.0)
    if aux.startswith("4"):
        return cre - deb
    if aux.startswith(("5", "6")):
        return -(deb - cre)
    return 0.0


def calcular_pyg_contable(contabilidad_df: pd.DataFrame, periodo_calc: Periodo) -> dict[str, object]:
    if contabilidad_df is None or contabilidad_df.empty:
        return {"ok": False, "mensaje": "No hay contabilidad cargada."}
    d = contabilidad_df[(contabilidad_df["Ano"] == int(periodo_calc.ano)) & (contabilidad_df["Mes"] == int(periodo_calc.mes_nro))].copy()
    if d.empty:
        return {"ok": False, "mensaje": f"La contabilidad cargada no tiene movimientos para {periodo_calc.etiqueta}."}

    def sum_ing(prefixes: tuple[str, ...]) -> float:
        base = d[d["Auxiliar"].str.startswith(prefixes)]
        return float((base["TotalCredito"] - base["TotalDebito"]).sum())

    def sum_gas(prefixes: tuple[str, ...]) -> float:
        base = d[d["Auxiliar"].str.startswith(prefixes)]
        return float((base["TotalDebito"] - base["TotalCredito"]).sum())

    ingresos_ordinarios = sum_ing(("412",))
    devoluciones = sum_gas(("417",))
    ingresos_netos = ingresos_ordinarios - devoluciones
    otros_ingresos = sum_ing(("429",))
    ingresos_financieros = sum_ing(("421",))
    costo_venta = sum_gas(("6",))
    gastos_admin = sum_gas(("51",))
    gastos_ventas = sum_gas(("52",))
    gastos_financieros = sum_gas(("530",))
    otros_gastos = sum_gas(("539",))
    impuesto_renta = sum_gas(("54",))
    ganancia_bruta = ingresos_netos - costo_venta
    utilidad_antes_imp = ganancia_bruta + otros_ingresos + ingresos_financieros - gastos_admin - gastos_ventas - otros_gastos - gastos_financieros
    utilidad_neta = utilidad_antes_imp - impuesto_renta

    resumen = pd.DataFrame([
        ["Ingresos de actividades ordinarias", ingresos_ordinarios, "P&G · clase 4"],
        ["Devoluciones", -devoluciones, "P&G · 417"],
        ["Ingresos netos", ingresos_netos, "Base de conciliación"],
        ["Costo de venta y/u operación", -costo_venta, "P&G · clase 6"],
        ["Ganancia bruta", ganancia_bruta, "Ingresos netos - costo de venta"],
        ["Otros ingresos", otros_ingresos, "P&G · 429"],
        ["Ingresos financieros / diferencia en cambio", ingresos_financieros, "P&G · 421"],
        ["Gastos de administración", -gastos_admin, "P&G · 51"],
        ["Gastos de ventas", -gastos_ventas, "P&G · 52"],
        ["Otros gastos", -otros_gastos, "P&G · 539"],
        ["Gastos financieros", -gastos_financieros, "P&G · 530"],
        ["Ganancia antes de impuesto", utilidad_antes_imp, "Resultado antes de renta"],
        ["Impuesto a las ganancias", -impuesto_renta, "P&G · 54"],
        ["Utilidad neta contable", utilidad_neta, "Resultado oficial estimado"],
    ], columns=["Concepto", "Valor", "Lectura"])

    detalle = d[d["Auxiliar"].str.startswith(("4", "5", "6"))].copy()
    detalle["Movimiento P&G"] = detalle.apply(_movimiento_pyg, axis=1)
    detalle = detalle.sort_values("Movimiento P&G", key=lambda s: s.abs(), ascending=False)

    balance_control = d[d["Auxiliar"].str.startswith(("1", "2", "3"))].copy()
    balance_control["Valor absoluto mayor"] = balance_control[["TotalDebito", "TotalCredito", "Saldo"]].abs().max(axis=1)
    balance_control = balance_control.sort_values("Valor absoluto mayor", ascending=False)
...
def candidatos_cercanos_contabilidad(contabilidad_df: pd.DataFrame, periodo_calc: Periodo, monto_objetivo: float, tolerancia_pct: float = 0.15) -> pd.DataFrame:
    if contabilidad_df is None or contabilidad_df.empty or abs(monto_objetivo) < 1e-9:
        return pd.DataFrame(columns=["Auxiliar", "NombreAux", "Clase", "TotalDebito", "TotalCredito", "Saldo", "Valor candidato", "Diferencia vs brecha", "Lectura"])
    d = contabilidad_df[(contabilidad_df["Ano"] == int(periodo_calc.ano)) & (contabilidad_df["Mes"] == int(periodo_calc.mes_nro))].copy()
    if d.empty:
        return pd.DataFrame(columns=["Auxiliar", "NombreAux", "Clase", "TotalDebito", "TotalCredito", "Saldo", "Valor candidato", "Diferencia vs brecha", "Lectura"])
    objetivo = abs(float(monto_objetivo))
    rows = []
    for _, r in d.iterrows():
        vals = [abs(float(r.get("TotalDebito", 0.0))), abs(float(r.get("TotalCredito", 0.0))), abs(float(r.get("Saldo", 0.0)))]
        val = min(vals, key=lambda x: abs(x - objetivo)) if vals else 0.0
        diff = val - objetivo
        if objetivo > 0 and abs(diff) <= objetivo * tolerancia_pct:
            clase = str(r.get("Clase", ""))
            lectura = "P&G" if clase in {"4", "5", "6"} else "Balance · control, no gasto"
            rows.append({
                "Auxiliar": r.get("Auxiliar", ""),
                "NombreAux": r.get("NombreAux", ""),
                "Clase": clase,
                "TotalDebito": r.get("TotalDebito", 0.0),
                "TotalCredito": r.get("TotalCredito", 0.0),
                "Saldo": r.get("Saldo", 0.0),
                "Valor candidato": val,
                "Diferencia vs brecha": diff,
                "Lectura": lectura,
            })
    out = pd.DataFrame(rows)
    if out.empty:
        return out
    return out.sort_values("Diferencia vs brecha", key=lambda s: s.abs()).reset_index(drop=True)


def analizar_extraordinarios_duplicados(df_mes_actual: pd.DataFrame) -> pd.DataFrame:
    """Detecta gastos extraordinarios que también aparecen en la base recurrente del mismo periodo."""
    if df_mes_actual is None or df_mes_actual.empty:
        return pd.DataFrame(columns=["Observacion", "Extraordinario", "Base recurrente misma observación", "Riesgo"])
    extra = df_mes_actual[df_mes_actual["Prod_norm"].str.contains("GASTOS EXTRA", na=False)].copy()
    base = df_mes_actual[~df_mes_actual["Prod_norm"].str.contains("GASTOS EXTRA", na=False)].copy()
    if extra.empty:
        return pd.DataFrame(columns=["Observacion", "Extraordinario", "Base recurrente misma observación", "Riesgo"])
    e = extra.groupby(["Obs_norm", "Observacion"], as_index=False)["Valor"].sum().rename(columns={"Valor": "Extraordinario"})
    b = base.groupby("Obs_norm", as_index=False)["Valor"].sum().rename(columns={"Valor": "Base recurrente misma observación"})
    out = e.merge(b, on="Obs_norm", how="left")
    out["Base recurrente misma observación"] = out["Base recurrente misma observación"].fillna(0.0)
    out["Riesgo"] = np.where(out["Base recurrente misma observación"].abs() > 1e-9, "Posible doble castigo", "Extraordinario único")
    return out[["Observacion", "Extraordinario", "Base recurrente misma observación", "Riesgo"]].sort_values("Extraordinario", ascending=False)
...
uploaded_contabilidad = st.sidebar.file_uploader("Cargar contabilidad / P&G (opcional)", type=["xlsx", "xlsm"], key="contabilidad_upload")
if uploaded is None:
...
contabilidad_df = pd.DataFrame()
contabilidad_error = ""
if uploaded_contabilidad is not None:
    try:
        contabilidad_df = load_contabilidad(uploaded_contabilidad.getvalue())
        st.sidebar.success("Contabilidad cargada para conciliación.")
    except Exception as exc:
        contabilidad_error = str(exc)
        st.sidebar.error(f"No pude leer contabilidad: {exc}")
...
dif_cambio_no_realizada = suma_diferencia_cambio_no_realizada(df_mes)
dif_cambio_realizada = suma_diferencia_cambio_realizada(df_mes)
diferencia_cambio_df = pd.DataFrame([
    ["Diferencia en cambio realizada", dif_cambio_realizada, safe_div(dif_cambio_realizada, und_vendida), safe_div(dif_cambio_realizada, kg_vendido), "Financiero normal", "Incluida en utilidad recurrente si viene dentro de C FIN o gastos financieros."],
    ["Diferencia en cambio no realizada", dif_cambio_no_realizada, safe_div(dif_cambio_no_realizada, und_vendida), safe_div(dif_cambio_no_realizada, kg_vendido), "Financiero no realizado", "Separada del costo recurrente; afecta utilidad final contable, no precio base."],
], columns=["Concepto", "Valor total", "$/saco vendido", "$/kg vendido", "Clasificación", "Tratamiento gerencial"])
...
# Utilidad unitaria recurrente: precio - costo comercial recurrente del producto.
# Los extraordinarios se separan para no contaminar el margen operativo normal.
utilidad_saco = precio_actual - costo_total_saco_sin_extra
utilidad_saco_con_extra = precio_actual - costo_total_saco_sin_extra - safe_div(gastos_extra, und_vendida)
margen_real = safe_div(utilidad_saco, precio_actual)
margen_real_con_extra_unitario = safe_div(utilidad_saco_con_extra, precio_actual)
...
# Regla CFO: venta real se compara contra costo de ventas real, no contra todo lo producido.
# La producción no vendida queda como inventario económico; si ventas > producción, se revela venta desde inventario previo.
venta_total_real = und_vendida * precio_actual
costo_total_produccion_sin_extra = costo_emp + costos_gastos
costo_total_produccion_con_extra = costo_total_produccion_sin_extra + gastos_extra
costo_ventas_real_sin_extra = und_vendida * costo_total_saco_sin_extra
costo_ventas_real_con_extra = costo_ventas_real_sin_extra + gastos_extra
costo_total_real_sin_extra = costo_ventas_real_sin_extra
costo_total_real_con_extra = costo_ventas_real_con_extra
utilidad_total_real = venta_total_real - costo_ventas_real_sin_extra
utilidad_total_real_con_extra = venta_total_real - costo_ventas_real_con_extra
...
utilidad_total_real_ton = safe_div(utilidad_total_real, ton_vendida)
utilidad_total_real_ton_con_extra = safe_div(utilidad_total_real_con_extra, ton_vendida)

utilidad_total_final_contable = utilidad_total_real_con_extra - dif_cambio_no_realizada
margen_total_final_contable = safe_div(utilidad_total_final_contable, venta_total_real)
utilidad_total_final_contable_ton = safe_div(utilidad_total_final_contable, ton_vendida)

bolsas_producidas_no_vendidas = max(und_emp - und_vendida, 0.0)
bolsas_vendidas_inventario_previo = max(und_vendida - und_emp, 0.0)
kg_producidos_no_vendidos = bolsas_producidas_no_vendidas * PESO_BOLSA_KG
kg_vendidos_inventario_previo = bolsas_vendidas_inventario_previo * PESO_BOLSA_KG
ton_producidas_no_vendidas = safe_div(kg_producidos_no_vendidos, 1000.0)
ton_vendidas_inventario_previo = safe_div(kg_vendidos_inventario_previo, 1000.0)
costo_inventario_no_vendido = bolsas_producidas_no_vendidas * costo_total_saco_sin_extra
saldo_neto_bolsas_produccion_venta = und_emp - und_vendida
saldo_neto_ton_produccion_venta = safe_div(kg_emp - kg_vendido, 1000.0)
...
def suma_energia(df_in: pd.DataFrame) -> float:
    base = filas_energia(df_in)
    return float(base["Valor"].sum()) if not base.empty else 0.0


def tendencia_simple(series: pd.Series) -> tuple[float, float]:
    serie = pd.to_numeric(series, errors="coerce").dropna()
    if len(serie) < 2:
        return 0.0, 0.0
    first = float(serie.iloc[0])
    last = float(serie.iloc[-1])
    return last - first, safe_div(last - first, first)
...
def serie_mensual_kpis(df_in: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, r in periodos.iterrows():
        ...
        dif_cambio_realizada_mes = suma_diferencia_cambio_realizada(d)
        dif_cambio_no_realizada_mes = suma_diferencia_cambio_no_realizada(d)
        ...
        costo_granel_kg = safe_div(cg, kg_g)
        costo_granel_ton = costo_granel_kg * 1000.0
        kg_ref_energia = kg_e if kg_e > 0 else kg_g
        ...
        costo_produccion_total_mes = ce + gastos
        costo_ventas_real_mes = und_vendida_mes * costo_comercial
        costo_ventas_real_con_extra_mes = costo_ventas_real_mes + gastos_extra_mes
        utilidad_empresa_mes = venta_total_mes - costo_ventas_real_mes
        utilidad_empresa_con_extra_mes = venta_total_mes - costo_ventas_real_con_extra_mes
        utilidad_empresa_final_contable_mes = utilidad_empresa_con_extra_mes - dif_cambio_no_realizada_mes
        ...
        rows.append({
            ...
            "Costo comercial total": costo_ventas_real_mes,
            "Costo producción total": costo_produccion_total_mes,
            "Gastos extraordinarios": gastos_extra_mes,
            "Diferencia cambio realizada": dif_cambio_realizada_mes,
            "Diferencia cambio no realizada": dif_cambio_no_realizada_mes,
            "Utilidad empresa": utilidad_empresa_mes,
            "Utilidad empresa con extra": utilidad_empresa_con_extra_mes,
            "Utilidad final contable": utilidad_empresa_final_contable_mes,
            "Utilidad / ton": safe_div(utilidad_empresa_mes, ton_vendida_mes),
            "Margen real": margen,
            "Margen con extra": safe_div(utilidad_empresa_con_extra_mes, venta_total_mes),
            "Margen final contable": safe_div(utilidad_empresa_final_contable_mes, venta_total_mes),
            "Costo granel": cg,
            "Kg granel": kg_g,
            "Costo granel / kg": costo_granel_kg,
            "Costo granel / ton": costo_granel_ton,
            "Energía": energia,
            "Energía / kg": safe_div(energia, kg_ref_energia),
            "Energía / ton": safe_div(energia, ton_ref_energia),
...
def _sumar_gastos_producto(df_producto_mes: pd.DataFrame, producto: ProductoCosteo, indices_gastos: list[str]) -> float:
    """Suma gastos recurrentes sin contaminar el costo con FX no realizado.

    La diferencia en cambio no realizada se reporta debajo de la utilidad recurrente;
    no debe mover el precio base del cemento.
    """
    if df_producto_mes is None or df_producto_mes.empty:
        return 0.0
    total = suma_indices(df_producto_mes, indices_gastos)
    total -= suma_diferencia_cambio_no_realizada(df_producto_mes, indices_gastos)
    return float(total)
...
def calcular_resumen_producto(
    ...
    dif_cambio_realizada_p = suma_diferencia_cambio_realizada(df_producto_mes, indices_gastos)
    dif_cambio_no_realizada_p = suma_diferencia_cambio_no_realizada(df_producto_mes, indices_gastos)
    ...
    utilidad_final_contable_p = utilidad_total_p - dif_cambio_no_realizada_p
    ...
    margen_final_contable_p = safe_div(utilidad_final_contable_p, venta_total_p)
    ...
    return {
        ...
        "Diferencia cambio realizada": dif_cambio_realizada_p,
        "Diferencia cambio no realizada": dif_cambio_no_realizada_p,
        ...
        "Utilidad final contable": utilidad_final_contable_p,
        "Utilidad / bolsa": utilidad_saco_p,
        "Utilidad / kg": utilidad_kg_p,
        "Utilidad / ton": utilidad_ton_p,
        "Margen": margen_p,
        "Margen final contable": margen_final_contable_p,
        "Estado": estado,
...
def _gastos_detalle_producto(
    ...
    if df_producto_mes is None or df_producto_mes.empty:
        return {
            ...
            "Ventas": 0.0,
            "Financieros": 0.0,
            "Diferencia cambio no realizada": 0.0,
            "Diferencia cambio realizada": 0.0,
            "Impuestos base": 0.0,
            "Impuesto renta": 0.0,
    ...
    dif_cambio_no_realizada = suma_diferencia_cambio_no_realizada(df_producto_mes, indices_gastos)
    dif_cambio_realizada = suma_diferencia_cambio_realizada(df_producto_mes, indices_gastos)
    ...
    if _producto_modo_subtotales(producto):
        ...
        financieros -= dif_cambio_no_realizada
    ...
    return {
        ...
        "Diferencia cambio no realizada": float(dif_cambio_no_realizada),
        "Diferencia cambio realizada": float(dif_cambio_realizada),
...
def calcular_componentes_costeo_producto(
    ...
    rows.append(unit_row("Gastos asignables", "Financieros recurrentes", gastos["Financieros"], "Financiero normal", " | ".join(producto.indices_fin or [])))
    rows.append(unit_row("Financiero separado", "Diferencia cambio realizada", gastos.get("Diferencia cambio realizada", 0.0), "Financiero normal", "DIFERENCIA EN CAMBIO", False, "Informativo", "La realizada puede estar incluida en financieros recurrentes si el índice la trae dentro de C FIN."))
    rows.append(unit_row("Financiero separado", "Diferencia cambio no realizada", gastos.get("Diferencia cambio no realizada", 0.0), "Financiero no realizado", "DIFERENCIA EN CAMBIO NO REALIZADA", False, "Informativo", "No suma al costo recurrente ni al precio base; se muestra en utilidad final contable."))
    ...
    rows.append(unit_row("Subtotal", "Gastos asignables total", gastos_total, "Suma ADM + ventas + financieros recurrentes + impuestos aplicados", " + ".join(idx_gastos), True, "Subtotal"))
...
# ------------------------------------------------------------
# Tabs independientes por producto: UG y ART
# ------------------------------------------------------------

def _buscar_producto_tabs(productos: dict[str, ProductoCosteo], token: str) -> ProductoCosteo | None:
    """Busca un producto por key, nombre o nombre corto para construir pestañas fijas por producto."""
    token_norm = norm_text(token)
    # Prioridad 1: key que empieza con el token.
    for key, cfg in productos.items():
        if norm_text(key).startswith(token_norm):
            return cfg
    # Prioridad 2: nombre corto o nombre contiene el token.
    for cfg in productos.values():
        texto = norm_text(f"{cfg.key} {cfg.nombre_corto} {cfg.nombre}")
        if token_norm in texto:
            return cfg
    return None


def _tab_label_producto(etapa: str, producto: ProductoCosteo | None, fallback: str) -> str:
    nombre = producto.nombre_corto if producto is not None and str(producto.nombre_corto).strip() else fallback
    return f"{etapa} {nombre}"


def _producto_periodo_df(producto: ProductoCosteo | None, periodo_calc: Periodo) -> pd.DataFrame:
    if producto is None:
        return pd.DataFrame()
    base = filtrar_producto_costeo(df_consolidado_completo, producto)
    return filtro_periodo(base, periodo_calc) if base is not None and not base.empty else pd.DataFrame()


def _componentes_granel_producto(producto: ProductoCosteo | None, periodo_calc: Periodo) -> dict[str, float | pd.DataFrame | list[str]]:
    d = _producto_periodo_df(producto, periodo_calc)
    if producto is None or d.empty:
        return {"df": d, "mp": 0.0, "mo": 0.0, "cif": 0.0, "total": 0.0, "kg": 0.0, "costo_kg": 0.0, "indices": []}
    indices = list(producto.indices_granel or [])
    mp = suma_indices(d, [indices[0]]) if len(indices) > 0 else 0.0
    mo = suma_indices(d, [indices[1]]) if len(indices) > 1 else 0.0
    cif = suma_indices(d, [indices[2]]) if len(indices) > 2 else 0.0
    total = mp + mo + cif
    kg = suma_obs(d, producto.obs_kg_granel)
    return {"df": d, "mp": mp, "mo": mo, "cif": cif, "total": total, "kg": kg, "costo_kg": safe_div(total, kg), "indices": indices}


def _componentes_empacado_producto(producto: ProductoCosteo | None, periodo_calc: Periodo) -> dict[str, float | pd.DataFrame | list[str]]:
    d = _producto_periodo_df(producto, periodo_calc)
    if producto is None or d.empty:
        return {"df": d, "mp": 0.0, "mo": 0.0, "cif": 0.0, "total": 0.0, "und": 0.0, "kg": 0.0, "costo_saco": 0.0, "costo_kg": 0.0, "transferido": 0.0, "incremental": 0.0, "incremental_saco": 0.0, "incremental_kg": 0.0, "indices": []}
    indices = list(producto.indices_empacado or [])
    peso = float(producto.peso_bolsa_kg or 0) or 50.0
    mp = suma_indices(d, [indices[0]]) if len(indices) > 0 else 0.0
    mo = suma_indices(d, [indices[1]]) if len(indices) > 1 else 0.0
    cif = suma_indices(d, [indices[2]]) if len(indices) > 2 else 0.0
    total = mp + mo + cif
    und = suma_obs(d, producto.obs_und_empacado)
    kg = und * peso
    transferido = suma_obs(d, producto.obs_cemento_transferido, indices)
    incremental = total - transferido
    return {
        "df": d, "mp": mp, "mo": mo, "cif": cif, "total": total, "und": und, "kg": kg,
        "costo_saco": safe_div(total, und), "costo_kg": safe_div(total, kg),
        "transferido": transferido, "incremental": incremental,
        "incremental_saco": safe_div(incremental, und), "incremental_kg": safe_div(incremental, kg),
        "indices": indices,
    }


def render_tab_granel_producto(producto: ProductoCosteo | None, token: str):
    if producto is None:
        st.warning(f"No encontré el producto {token} en Parametros Productos o en Consolidado.")
        return
    actual = _componentes_granel_producto(producto, periodo)
    previo = _componentes_granel_producto(producto, periodo_comp)
    d = actual["df"]
    if d is None or d.empty:
        st.warning(f"No hay datos de granel para {producto.nombre} en {periodo.etiqueta}.")
        return

    var_costo_kg, delta_costo_kg = variacion(float(actual["costo_kg"]), float(previo["costo_kg"]), periodo_comp.etiqueta)
    st.subheader(f"Granel {producto.nombre_corto} - {periodo.etiqueta}")
    st.caption("Costeo independiente del granel del producto. No mezcla UG con ART ni usa promedios de portafolio.")

    c1, c2, c3, c4 = st.columns(4)
    with c1:
        kpi("Costo total granel", money(float(actual["total"])))
    with c2:
        kpi("Kg producidos granel", num(float(actual["kg"]), 0))
    with c3:
        kpi("Costo / kg", money(float(actual["costo_kg"])), delta_costo_kg)
    with c4:
        kpi("CIF / costo granel", pct(safe_div(float(actual["cif"]), float(actual["total"]))))

    col_a, col_b = st.columns(2)
    with col_a:
        waterfall(
            f"Waterfall costo granel {producto.nombre_corto}",
            ["MP", "MO", "CIF"],
            [float(actual["mp"]), float(actual["mo"]), float(actual["cif"])],
            "Total",
        )
    with col_b:
        comp = pd.DataFrame({"Componente": ["MP", "MO", "CIF"], "Valor": [float(actual["mp"]), float(actual["mo"]), float(actual["cif"])]})
        fig_pie = px.pie(comp, names="Componente", values="Valor", title=f"Composición granel {producto.nombre_corto}",
                         color_discrete_sequence=["#E8650A", "#F5A623", "#2DBD6E"])
        fig_pie.update_traces(textfont_color="#080D18", pull=[0.04, 0, 0])
        st.plotly_chart(fig_pie, use_container_width=True)

    emp = _componentes_empacado_producto(producto, periodo)
    pareto_g = build_pareto(resumen_por_observacion(d, producto.indices_granel), float(emp["und"]), float(emp["kg"]))
    pareto_chart(pareto_g, f"Pareto granel {producto.nombre_corto} por observación", top_n)
    dataframe_gerencial(pareto_g)


def render_tab_empacado_producto(producto: ProductoCosteo | None, token: str):
    if producto is None:
        st.warning(f"No encontré el producto {token} en Parametros Productos o en Consolidado.")
        return
    actual = _componentes_empacado_producto(producto, periodo)
    previo = _componentes_empacado_producto(producto, periodo_comp)
    d = actual["df"]
    if d is None or d.empty:
        st.warning(f"No hay datos de empacado para {producto.nombre} en {periodo.etiqueta}.")
        return

    var_costo_saco, delta_costo_saco_prod = variacion(float(actual["costo_saco"]), float(previo["costo_saco"]), periodo_comp.etiqueta)
    st.subheader(f"Empacado {producto.nombre_corto} - {periodo.etiqueta}")
    st.caption("Costeo independiente del empacado del producto. El saco, precio y cantidades respetan la presentación propia.")

    c1, c2, c3, c4 = st.columns(4)
    with c1:
        kpi("Costo empacado completo", money(float(actual["total"])))
    with c2:
        kpi("UND producidas", num(float(actual["und"]), 0))
    with c3:
        kpi("Costo / saco", money(float(actual["costo_saco"])), delta_costo_saco_prod)
    with c4:
        kpi("Costo / kg", money(float(actual["costo_kg"])))

    c1, c2, c3, c4 = st.columns(4)
    with c1:
        kpi("Cemento transferido", money(float(actual["transferido"])))
    with c2:
        kpi("Incremental empaque", money(float(actual["incremental"])))
    with c3:
        kpi("Incremental / saco", money(float(actual["incremental_saco"])))
    with c4:
        kpi("Incremental / kg", money(float(actual["incremental_kg"])))

    col_a, col_b = st.columns(2)
    with col_a:
        waterfall(
            f"Waterfall empacado {producto.nombre_corto}",
            ["Cemento granel", "MP/Empaque", "MO", "CIF"],
            [float(actual["transferido"]), float(actual["mp"]) - float(actual["transferido"]), float(actual["mo"]), float(actual["cif"])],
            "Total",
        )
    with col_b:
        comp = pd.DataFrame({
            "Componente": ["Cemento granel", "MP/Empaque", "MO", "CIF"],
            "Valor": [float(actual["transferido"]), float(actual["mp"]) - float(actual["transferido"]), float(actual["mo"]), float(actual["cif"])],
        })
        fig_pie2 = px.pie(comp, names="Componente", values="Valor", title=f"Composición empacado {producto.nombre_corto}",
                          color_discrete_sequence=["#E8650A", "#F5A623", "#2DBD6E", "#F59E0B"])
        fig_pie2.update_traces(textfont_color="#080D18", pull=[0.04, 0, 0, 0])
        st.plotly_chart(fig_pie2, use_container_width=True)

    pareto_e = build_pareto(resumen_por_observacion(d, producto.indices_empacado), float(actual["und"]), float(actual["kg"]))
    pareto_chart(pareto_e, f"Pareto empacado {producto.nombre_corto} por observación", top_n)
    dataframe_gerencial(pareto_e)


producto_ug_tabs = _buscar_producto_tabs(productos_disponibles, "UG")
producto_art_tabs = _buscar_producto_tabs(productos_disponibles, "ART")
...
tabs = st.tabs([
    "📊 Resumen Gerencial",
    "💰 Precio & Margen",
    f"🏭 {_tab_label_producto('Granel', producto_ug_tabs, 'UG 50 kg')}",
    f"📦 {_tab_label_producto('Empacado', producto_ug_tabs, 'UG 50 kg')}",
    f"🏭 {_tab_label_producto('Granel', producto_art_tabs, 'ART 42.5 kg')}",
    f"📦 {_tab_label_producto('Empacado', producto_art_tabs, 'ART 42.5 kg')}",
    "🧾 Gastos ADM/Ventas",
    "📈 Paretos",
    "↕️ Variaciones",
    "📉 Evolución Mensual",
...
with tabs[0]:
    ...
    if resumen_productos_df.empty:
        ...
        with p2:
            kpi("Costo comercial recurrente", money(costo_portafolio), help_text="No es P&G completo ni utilidad neta")
        with p3:
            kpi("Utilidad comercial recurrente", money(utilidad_portafolio), help_text=f"{money(utilidad_ton_portafolio)}/ton · antes de extraordinarios", tone="red" if utilidad_portafolio < 0 else "green")
        with p4:
            kpi("Margen comercial recurrente", pct(margen_portafolio), tone="red" if margen_portafolio < 0 else "green")

        st.warning("Lectura CFO: este bloque es rentabilidad comercial recurrente del costeo. No lo llames utilidad neta hasta conciliar contra el P&G contable.")
        ...
        columnas_resumen = [
            ...
            "Venta total", "Costo ventas real", "Utilidad empresa", "Diferencia cambio no realizada", "Utilidad final contable", "Margen", "Margen final contable", "Costo producción total", "Bolsas producidas no vendidas", "Bolsas vendidas inventario previo", "Costo inventario no vendido", "Participación venta", "Participación utilidad", "Estado",
        ]
...
with tabs[1]:
    ...
    if precio_productos_df.empty:
        ...
        columnas_precio = [
            ...
            "Brecha vs objetivo", "Margen", "Diferencia margen", "Venta total", "Costo ventas real", "Utilidad empresa", "Diferencia cambio no realizada", "Utilidad final contable", "Costo producción total", "Costo inventario no vendido", "Sin ventas", "Estado",
        ]
...
with tabs[2]:
    render_tab_granel_producto(producto_ug_tabs, "UG")


with tabs[3]:
    render_tab_empacado_producto(producto_ug_tabs, "UG")


with tabs[4]:
    render_tab_granel_producto(producto_art_tabs, "ART")


with tabs[5]:
    render_tab_empacado_producto(producto_art_tabs, "ART")
with tabs[6]:
...
with tabs[9]:
    ...
    if kpis_mensuales.empty or len(kpis_mensuales) < 2:
        ...
        util_cols = ["Periodo", "Venta total empresa", "Costo comercial total", "Utilidad empresa", "Utilidad empresa con extra", "Diferencia cambio no realizada", "Utilidad final contable", "Utilidad / ton", "Margen real", "Margen con extra", "Margen final contable"]
        ...
        with u3:
            kpi("Utilidad final contable", money(ultimo_util.get("Utilidad final contable", 0)), help_text="Incluye FX no realizado", tone="red" if ultimo_util.get("Utilidad final contable", 0) < 0 else "green")
        with u4:
            kpi("Margen final contable", pct(ultimo_util.get("Margen final contable", 0)), help_text=str(ultimo_util.get("Periodo", "")), tone="red" if ultimo_util.get("Margen final contable", 0) < 0 else "green")
        ...
        util_long = kpis_mensuales.melt(
            id_vars=["Periodo", "PeriodoOrden"],
            value_vars=["Venta total empresa", "Costo comercial total", "Utilidad empresa", "Utilidad final contable"],
            var_name="Métrica",
            value_name="Valor",
        ).sort_values("PeriodoOrden")
        ...
        st.caption(
            ...
            "costo granel, costo empacado, energía y energía unitaria."
        )
        ...
        micro_specs = [spec for spec in micro_specs if spec[0] in kpis_mensuales.columns]

        resumen_micro = []
        for col_metric, titulo, lectura in micro_specs:
            serie = pd.to_numeric(kpis_mensuales[col_metric], errors="coerce").fillna(0)
            actual = float(serie.iloc[-1]) if len(serie) else 0.0
            diff, rel = tendencia_simple(serie)
            resumen_micro.append([titulo, actual, diff, rel, lectura])

        resumen_micro_df = pd.DataFrame(
            resumen_micro,
            columns=["Indicador", "Último valor", "Cambio vs primer mes", "Cambio %", "Lectura"]
        )

        if resumen_micro_df.empty:
            st.info("No hay métricas suficientes para los micrográficos ejecutivos.")
        else:
            ...
            for start_i in range(0, len(micro_specs), 3):
                ...
                for col_ui, (col_metric, titulo, lectura) in zip(cols, micro_specs[start_i:start_i+3]):
                    with col_ui:
                        ...
                        if float(mini[col_metric].abs().sum()) <= 0:
                            st.info(f"Sin datos para {titulo}.")
                        else:
                            fig = px.line(mini, x="Periodo", y=col_metric, markers=True, title=titulo)
                            fig.update_traces(line_width=2.7, marker_size=7)
                            fig.update_layout(height=300, margin=dict(l=25, r=20, t=58, b=75), xaxis_tickangle=-35, showlegend=False)
                            st.plotly_chart(fig, use_container_width=True)
        ...
        if energia_detalle.empty:
            ...
            energia_resumen = (
                ...
                .groupby(["Periodo", "PeriodoOrden", "Observacion"], as_index=False)["Valor"]
                .sum()
                .sort_values(["PeriodoOrden", "Valor"], ascending=[True, False])
            )
            fig = px.bar(
                energia_resumen,
                x="Periodo",
                y="Valor",
                color="Observacion",
                title="Composición mensual del costo de energía",
            )
            fig.update_layout(height=430, xaxis_tickangle=-35, legend_title_text="Rubro energía")
            st.plotly_chart(fig, use_container_width=True)
            dataframe_gerencial(energia_resumen)

        st.markdown("### Tabla mensual de KPIs clave")
        kpis_cols_clave = [
...
with tabs[12]:
    ...
    paso_gastos = pd.DataFrame([
        ...
        ["Financieros recurrentes", "C FIN sin diferencia en cambio no realizada", suma_financiera_comercial(df_mes), safe_div(suma_financiera_comercial(df_mes), und_emp), safe_div(suma_financiera_comercial(df_mes), kg_emp)],
    ...
    st.markdown("### Diferencia en cambio")
    st.caption("La realizada puede ser financiero normal; la no realizada se separa del costo recurrente y no debe mover el precio base.")
    dataframe_gerencial(diferencia_cambio_df)
    ...
    puente_comercial = pd.DataFrame([
        ...
        ["D", "Gastos extraordinarios / saco producido", "Gastos ExtraOrdinarios / UND PRODUCIDAS Q", gastos_extra_saco],
        ["E = C + D", "Costo total comercial con extraordinarios / saco producido", "Costo comercial sin extra + gasto extraordinario / saco producido", costo_total_saco_con_extra],
        ["F", "Precio actual / saco vendido", OBS_PRECIO_BOLSA_USADA, precio_actual],
        ["G = F - C", "Utilidad recurrente / saco vendido", "Precio actual - costo total comercial sin extraordinarios", utilidad_saco],
        ["H = G / F", "Margen recurrente", "Utilidad recurrente / saco vendido ÷ precio actual / saco vendido", margen_real],
        ["I", "Gastos extraordinarios / saco vendido", "Gastos ExtraOrdinarios / UND VENDIDAS", safe_div(gastos_extra, und_vendida)],
        ["J = F - C - I", "Utilidad con extraordinarios / saco vendido", "Precio actual - costo comercial recurrente - extraordinarios unitizados sobre ventas", utilidad_saco_con_extra],
        ["K", "Diferencia cambio no realizada / saco vendido", "Diferencia cambio no realizada / UND VENDIDAS", safe_div(dif_cambio_no_realizada, und_vendida)],
        ["L = J - K", "Utilidad final contable / saco vendido", "Utilidad con extraordinarios - diferencia cambio no realizada", safe_div(utilidad_total_final_contable, und_vendida)],
    ...
    st.markdown(
        ...
        - La diferencia en cambio no realizada se muestra en financiero separado: afecta la utilidad final contable, pero no el costo recurrente ni el precio base del saco.
        """
    )
...
with tabs[14]:
    st.subheader("Utilidad de la empresa / Conciliación rápida")
    st.caption("Vista CFO: utilidad real del mes seleccionado, conciliación contra P&G contable y utilidad proyectada por toneladas para el producto activo.")

    # Bloque visible: si la contabilidad está cargada, la conciliación aparece aquí mismo.
    # Antes estaba solo en una pestaña al extremo derecho y el usuario podía no verla por el scroll horizontal de tabs.
    if contabilidad_error:
        st.error(f"La contabilidad cargada no pudo leerse: {contabilidad_error}")
    elif contabilidad_df.empty:
        st.info("Para ver la conciliación contable, carga el archivo de contabilidad/P&G en la barra lateral.")
    else:
        pyg_util = calcular_pyg_contable(contabilidad_df, periodo)
        if not pyg_util.get("ok"):
            st.warning(str(pyg_util.get("mensaje", "No hay datos contables para conciliar.")))
        else:
            utilidad_contable_util = float(pyg_util["utilidad_neta"])
            ingresos_contables_util = float(pyg_util["ingresos_netos"])
            diferencia_utilidad_util = utilidad_contable_util - utilidad_total_real_con_extra
            diferencia_ventas_util = ingresos_contables_util - venta_total_real
            diferencia_no_ventas_util = diferencia_utilidad_util - diferencia_ventas_util
            st.markdown("### Conciliación rápida contra P&G contable")
            q1, q2, q3, q4 = st.columns(4)
            with q1:
                kpi("Utilidad gerencial con extraordinarios", money(utilidad_total_real_con_extra), help_text="Desde costeo app", tone="red" if utilidad_total_real_con_extra < 0 else "green")
            with q2:
                kpi("Utilidad neta contable", money(utilidad_contable_util), help_text="Desde contabilidad/P&G", tone="red" if utilidad_contable_util < 0 else "green")
            with q3:
                kpi("Diferencia app vs contabilidad", money(diferencia_utilidad_util), help_text="Contabilidad - app", tone="yellow" if abs(diferencia_utilidad_util) > 1 else "green")
            with q4:
                kpi("Ingresos netos contables", money(ingresos_contables_util), help_text=f"Diferencia venta: {money(diferencia_ventas_util)}", tone="neutral")

            puente_rapido_util = pd.DataFrame([
                ["Utilidad gerencial con extraordinarios", utilidad_total_real_con_extra, "Resultado calculado por la app"],
                ["Diferencia ingresos netos contables vs venta app", diferencia_ventas_util, "Precio promedio, devoluciones, redondeos o ventas no mapeadas"],
                ["Diferencia restante: costos/gastos/financieros/impuestos", diferencia_no_ventas_util, "Partidas que la contabilidad mide distinto al costeo gerencial"],
                ["Utilidad neta contable conciliada", utilidad_total_real_con_extra + diferencia_ventas_util + diferencia_no_ventas_util, "Debe igualar el P&G"],
                ["Check", (utilidad_total_real_con_extra + diferencia_ventas_util + diferencia_no_ventas_util) - utilidad_contable_util, "Debe ser cero"],
            ], columns=["Paso", "Valor", "Lectura"])
            dataframe_gerencial(puente_rapido_util)
            st.caption("La tarjeta inferior de diferencia en cambio no realizada es una lectura gerencial; la utilidad neta oficial es la del P&G contable mostrado en este bloque.")
    ...
    st.markdown("### Resultado real del mes · venta, costo vendido e inventario")
    st.caption(
        "Sin extraordinarios = rentabilidad recurrente del negocio. Con extraordinarios = rentabilidad después de descontar eventos no recurrentes del periodo. "
        "La diferencia en cambio no realizada se separa del costo recurrente y solo afecta la utilidad final contable. "
        "La venta real se compara contra el costo de las bolsas vendidas, no contra todo lo producido."
    )

    r1, r2, r3, r4 = st.columns(4)
    with r1:
        kpi("Venta total real", money(venta_total_real), help_text=f"{num(und_vendida, 0)} bolsas vendidas · {num(ton_vendida, 2)} t")
    with r2:
        kpi("Costo de ventas real", money(costo_ventas_real_sin_extra), help_text="Costo unitario recurrente × bolsas vendidas")
    with r3:
        kpi("Utilidad sin extraordinarios", money(utilidad_total_real), help_text=f"{money(utilidad_total_real_ton)}/ton vendida", tone="red" if utilidad_total_real < 0 else "green")
    with r4:
    ...
    with r5:
        kpi("Margen sin extraordinarios", pct(margen_total_real), tone="red" if margen_total_real < 0 else "green")
    with r6:
        kpi("Costo producido del mes", money(costo_total_produccion_sin_extra), help_text=f"{num(und_emp, 0)} bolsas producidas · {num(tons_base_util, 2)} t")
    with r7:
        kpi("Costo producido no vendido", money(costo_inventario_no_vendido), help_text=f"{num(bolsas_producidas_no_vendidas, 0)} bolsas", tone="yellow" if costo_inventario_no_vendido > 0 else "green")
    with r8:
        kpi("Venta desde inventario previo", num(bolsas_vendidas_inventario_previo, 0), help_text=f"{num(ton_vendidas_inventario_previo, 2)} t", tone="yellow" if bolsas_vendidas_inventario_previo > 0 else "neutral")

    if bolsas_producidas_no_vendidas > 0 or bolsas_vendidas_inventario_previo > 0:
        st.info(
            f"Lectura CFO: producción y venta no coinciden. Quedaron {num(bolsas_producidas_no_vendidas, 0)} bolsas producidas no vendidas "
            f"y se vendieron {num(bolsas_vendidas_inventario_previo, 0)} bolsas desde inventario previo. "
            "Por eso la utilidad debe leerse sobre costo de ventas real, no sobre costo total producido."
        )

    fx1, fx2, fx3, fx4 = st.columns(4)
    with fx1:
        kpi("Diferencia cambio realizada", money(dif_cambio_realizada), help_text="Financiero normal si es recurrente", tone="yellow" if abs(dif_cambio_realizada) > 0 else "neutral")
    with fx2:
        kpi("Diferencia cambio no realizada", money(dif_cambio_no_realizada), help_text="Fuera del precio base", tone="yellow" if abs(dif_cambio_no_realizada) > 0 else "neutral")
    with fx3:
        kpi("Resultado gerencial con FX no realizada", money(utilidad_total_final_contable), help_text=f"No es utilidad neta P&G · margen: {pct(margen_total_final_contable)}", tone="red" if utilidad_total_final_contable < 0 else "yellow")
    with fx4:
        kpi("Resultado gerencial / ton", money(utilidad_total_final_contable_ton), help_text="Incluye FX no realizada; no es P&G oficial", tone="red" if utilidad_total_final_contable_ton < 0 else "yellow")

    st.markdown("### Diferencia en cambio")
    st.caption("Valor positivo reduce utilidad si está registrado como gasto; valor negativo la aumenta si está registrado como ingreso o reversión.")
    dataframe_gerencial(diferencia_cambio_df)

    utilidad_real_unitaria = pd.DataFrame([
        ["Venta real", "Ingreso", venta_total_real, safe_div(venta_total_real, ton_vendida), safe_div(venta_total_real, kg_vendido), safe_div(venta_total_real, und_vendida), "Solo unidades vendidas"],
        ["Costo de ventas sin extraordinarios", "Costo vendido", costo_ventas_real_sin_extra, safe_div(costo_ventas_real_sin_extra, ton_vendida), safe_div(costo_ventas_real_sin_extra, kg_vendido), safe_div(costo_ventas_real_sin_extra, und_vendida), "Costo recurrente aplicado a bolsas vendidas"],
        ["Utilidad sin extraordinarios", "Resultado recurrente", utilidad_total_real, safe_div(utilidad_total_real, ton_vendida), safe_div(utilidad_total_real, kg_vendido), safe_div(utilidad_total_real, und_vendida), "Indicador principal para precio y operación"],
        ["Gastos extraordinarios", "No recurrente", gastos_extra, safe_div(gastos_extra, ton_vendida), safe_div(gastos_extra, kg_vendido), safe_div(gastos_extra, und_vendida), "Se descuenta separado; no debe fijar precio recurrente"],
        ["Utilidad con extraordinarios", "Resultado después de extra", utilidad_total_real_con_extra, safe_div(utilidad_total_real_con_extra, ton_vendida), safe_div(utilidad_total_real_con_extra, kg_vendido), safe_div(utilidad_total_real_con_extra, und_vendida), "Sirve para caja/P&G del mes, no para tarifa normal"],
        ["Diferencia cambio realizada", "Financiero normal", dif_cambio_realizada, safe_div(dif_cambio_realizada, ton_vendida), safe_div(dif_cambio_realizada, kg_vendido), safe_div(dif_cambio_realizada, und_vendida), "Ya está dentro del financiero recurrente si el índice C FIN la trae"],
        ["Diferencia cambio no realizada", "Financiero no realizado", dif_cambio_no_realizada, safe_div(dif_cambio_no_realizada, ton_vendida), safe_div(dif_cambio_no_realizada, kg_vendido), safe_div(dif_cambio_no_realizada, und_vendida), "No mueve el precio base; afecta resultado final contable"],
        ["Resultado gerencial con FX no realizada", "Resultado gerencial", utilidad_total_final_contable, safe_div(utilidad_total_final_contable, ton_vendida), safe_div(utilidad_total_final_contable, kg_vendido), safe_div(utilidad_total_final_contable, und_vendida), "No es utilidad neta contable; para eso use la conciliación P&G"],
        ["Costo producido del mes", "Producción", costo_total_produccion_sin_extra, safe_div(costo_total_produccion_sin_extra, tons_base_util), safe_div(costo_total_produccion_sin_extra, kg_emp), safe_div(costo_total_produccion_sin_extra, und_emp), "Costo de fabricar todo lo producido"],
        ["Costo inventario no vendido", "Inventario económico", costo_inventario_no_vendido, safe_div(costo_inventario_no_vendido, ton_producidas_no_vendidas), safe_div(costo_inventario_no_vendido, kg_producidos_no_vendidos), safe_div(costo_inventario_no_vendido, bolsas_producidas_no_vendidas), "No es pérdida: es costo pendiente de monetizar"],
    ], columns=["Concepto", "Tipo", "Total empresa", "$/ton", "$/kg", "$/bolsa", "Lectura gerencial"])
    dataframe_gerencial(utilidad_real_unitaria)

    st.markdown("### Qué significa utilidad sin y con extraordinarios")
    st.markdown(
        """
        - **Utilidad sin extraordinarios:** venta real menos costo comercial recurrente de lo vendido. Es la métrica que debe mandar para precio, eficiencia operativa y comité comercial.
        - **Utilidad con extraordinarios:** la utilidad recurrente menos gastos extraordinarios del periodo. Sirve para leer caja y resultado mensual completo, pero no debe usarse para castigar el precio normal si el evento no se repite.
        - **Diferencia en cambio realizada:** financiero normal cuando nace de pagos, cobros o deudas reales recurrentes. No es extraordinaria por defecto.
        - **Diferencia en cambio no realizada:** financiero no realizado. No debe contaminar el costo del saco ni el precio base; se muestra en utilidad final contable.
        - **Costo producido no vendido:** costo de inventario económico. No es pérdida automática; se vuelve utilidad o pérdida cuando se venda, dependiendo del precio.
        """
    )
    ...
    # Corrección CFO: el margen proyectado es SIEMPRE utilidad / venta.
    # El margen meta no se suma ni se mezcla con el margen real; solo sirve para calcular
    # el precio requerido y para disparar la alerta de cumplimiento.
    costo_ton_util = safe_div(costo_total_util, toneladas_util)
    costo_saco_util = safe_div(costo_total_util, sacos_util)
    precio_saco_util = safe_div(precio_ton_util, sacos_por_ton_util)
    ...
    precio_obj_ton_util = safe_div(costo_ton_util, 1 - margen_meta_util)
    precio_obj_saco_util = safe_div(precio_obj_ton_util, sacos_por_ton_util)
    precio_obj_ton_util_iva = precio_obj_ton_util * (1 + iva)
    precio_obj_saco_util_iva = precio_obj_saco_util * (1 + iva)
    brecha_ton_util = precio_obj_ton_util - precio_ton_util
    brecha_saco_util = precio_obj_saco_util - precio_saco_util
    brecha_margen_pp_util = (margen_proyectado_empresa - margen_meta_util) * 100.0
    cumple_meta_util = margen_proyectado_empresa >= margen_meta_util
    estado_meta_util = "CUMPLE META" if cumple_meta_util else "NO CUMPLE META"
    tono_meta_util = "green" if cumple_meta_util else "red"

    g1, g2, g3, g4 = st.columns(4)
    with g1:
        kpi("Venta proyectada", money(venta_util), help_text=f"{num(sacos_util, 0)} bolsas · {num(toneladas_util, 2)} t")
    with g2:
        kpi("Costo total proyectado", money(costo_total_util), help_text=f"Costo/ton: {money(costo_ton_util)} · bolsa: {money(costo_saco_util)}")
    with g3:
    ...
    with g4:
        kpi("Margen proyectado", pct(margen_proyectado_empresa), delta=f"{num(brecha_margen_pp_util, 2)} pp vs meta", help_text=f"Meta: {pct(margen_meta_util)}", tone=tono_meta_util)

    m1, m2, m3, m4 = st.columns(4)
    with m1:
        kpi("Estado margen meta", estado_meta_util, help_text=f"Meta: {pct(margen_meta_util)} · Proyectado: {pct(margen_proyectado_empresa)}", tone=tono_meta_util)
    with m2:
        kpi("Precio requerido / tonelada", money(precio_obj_ton_util), delta=f"Brecha: {money(brecha_ton_util)}", help_text=f"Con IVA: {money(precio_obj_ton_util_iva)}", tone=tono_meta_util)
    with m3:
        kpi("Precio requerido / bolsa", money(precio_obj_saco_util), delta=f"Brecha: {money(brecha_saco_util)}", help_text=f"Con IVA: {money(precio_obj_saco_util_iva)}", tone=tono_meta_util)
    with m4:
        kpi("Precio actual simulado / bolsa", money(precio_saco_util), help_text=f"Precio/ton: {money(precio_ton_util)}", tone="neutral")

    if cumple_meta_util:
        st.success(
            f"La proyección cumple la meta: margen proyectado {pct(margen_proyectado_empresa)} vs meta {pct(margen_meta_util)}. "
            f"Colchón de precio: {fmt_money(abs(brecha_ton_util))}/ton."
        )
    else:
        st.error(
            f"No cumple margen meta: margen proyectado {pct(margen_proyectado_empresa)} vs meta {pct(margen_meta_util)}. "
            f"Debe subir {fmt_money(max(brecha_ton_util, 0.0))}/ton, equivalente a {fmt_money(max(brecha_saco_util, 0.0))}/bolsa antes de IVA."
        )

    venta_obj_util = toneladas_util * precio_obj_ton_util
    utilidad_obj_util = venta_obj_util - costo_total_util
    margen_obj_util_calc = safe_div(utilidad_obj_util, venta_obj_util)
    ...
    comparativo_utilidad = pd.DataFrame([
        ...
        ["Proyectado con precio ingresado", toneladas_util, sacos_util, venta_util, costo_total_util, utilidad_proyectada_empresa, margen_proyectado_empresa, safe_div(utilidad_proyectada_empresa, toneladas_util), safe_div(utilidad_proyectada_empresa, kg_util), safe_div(utilidad_proyectada_empresa, sacos_util)],
        ["Proyectado para cumplir meta", toneladas_util, sacos_util, venta_obj_util, costo_total_util, utilidad_obj_util, margen_obj_util_calc, safe_div(utilidad_obj_util, toneladas_util), safe_div(utilidad_obj_util, kg_util), safe_div(utilidad_obj_util, sacos_util)],
    ...
    brecha_precio_df = pd.DataFrame([
        ["Margen meta", pct(margen_meta_util), "-"],
        ["Margen proyectado con precio ingresado", pct(margen_proyectado_empresa), f"{num(brecha_margen_pp_util, 2)} pp"],
        ["Precio ingresado / ton", money(precio_ton_util), "-"],
        ["Precio requerido / ton", money(precio_obj_ton_util), money(brecha_ton_util)],
        ["Precio ingresado / bolsa", money(precio_saco_util), "-"],
        ["Precio requerido / bolsa", money(precio_obj_saco_util), money(brecha_saco_util)],
    ], columns=["Indicador", "Valor", "Brecha vs meta/precio ingresado"])
    dataframe_gerencial(brecha_precio_df)
...
with tabs[15]:
    ...
    brecha_precio_ton_sim = precio_obj_ton - precio_ton_sim
    brecha_precio_saco_sim = precio_obj_saco_sim - precio_saco_sim
    brecha_margen_pp_sim = (margen_sim - margen_meta_sim) * 100.0
    cumple_meta_sim = margen_sim >= margen_meta_sim
    estado_meta_sim = "CUMPLE META" if cumple_meta_sim else "NO CUMPLE META"
    tono_meta_sim = "green" if cumple_meta_sim else "red"
    ...
    with k3:
        kpi("Margen proyectado", pct(margen_sim), delta=f"{num(brecha_margen_pp_sim, 2)} pp vs meta", help_text=f"Meta: {pct(margen_meta_sim)}", tone=tono_meta_sim)
    with k4:
    ...
    with k2:
        kpi("Precio objetivo / tonelada", money(precio_obj_ton), delta=f"Brecha: {money(brecha_precio_ton_sim)}", help_text=f"Margen meta: {pct(margen_meta_sim)}", tone=tono_meta_sim)
    with k3:
        kpi("Precio objetivo / saco", money(precio_obj_saco_sim), delta=f"Brecha: {money(brecha_precio_saco_sim)}", help_text=f"kg objetivo: {money(precio_obj_kg_sim)}", tone=tono_meta_sim)
    with k4:
        kpi("Estado margen meta", estado_meta_sim, help_text=f"Precio + IVA / saco: {money(precio_obj_saco_iva)} · ton + IVA: {money(precio_obj_ton_iva)}", tone=tono_meta_sim)

    if cumple_meta_sim:
        st.success(
            f"El precio simulado cumple la meta: margen proyectado {pct(margen_sim)} vs meta {pct(margen_meta_sim)}. "
            f"Colchón de precio: {fmt_money(abs(brecha_precio_ton_sim))}/ton."
        )
    else:
        st.error(
            f"El precio simulado no cumple la meta: margen proyectado {pct(margen_sim)} vs meta {pct(margen_meta_sim)}. "
            f"Ajuste requerido: {fmt_money(max(brecha_precio_ton_sim, 0.0))}/ton · {fmt_money(max(brecha_precio_saco_sim, 0.0))}/saco antes de IVA."
        )
...
with tabs[17]:
    st.subheader("Conciliación P&G · contabilidad vs costeo gerencial")
    st.info("Si esta pestaña queda fuera de vista por la barra horizontal de tabs, la misma conciliación rápida aparece también en ‘Utilidad Empresa / Conciliación’.")
    st.caption(
        "Esta vista separa tres verdades: rentabilidad comercial recurrente, resultado gerencial con extraordinarios y utilidad neta contable. "
        "La regla de oro: cuentas 1/2/3 son balance; se muestran como control, pero no entran al P&G ni al costo del saco."
    )

    if contabilidad_error:
        st.error(f"La contabilidad cargada no pudo leerse: {contabilidad_error}")
    elif contabilidad_df.empty:
        st.info("Carga el archivo de contabilidad en la barra lateral para activar la conciliación contra P&G.")
        st.markdown(
            """
            **Diseño de gobierno financiero recomendado:**
            - **Utilidad comercial recurrente:** venta real menos costo comercial recurrente de lo vendido.
            - **Utilidad después de extraordinarios:** utilidad recurrente menos eventos no recurrentes definidos y soportados.
            - **Utilidad neta contable:** resultado oficial del P&G, después de otros ingresos/gastos, financieros e impuestos.
            """
        )
    else:
        pyg = calcular_pyg_contable(contabilidad_df, periodo)
        if not pyg.get("ok"):
            st.warning(str(pyg.get("mensaje", "No hay datos contables para conciliar.")))
        else:
            utilidad_contable = float(pyg["utilidad_neta"])
            ingresos_netos_contables = float(pyg["ingresos_netos"])
            diferencia_utilidad = utilidad_contable - utilidad_total_real_con_extra
            diferencia_ventas = ingresos_netos_contables - venta_total_real
            diferencia_restante = diferencia_utilidad - diferencia_ventas

            c1, c2, c3, c4 = st.columns(4)
            with c1:
                kpi("Utilidad gerencial con extraordinarios", money(utilidad_total_real_con_extra), help_text="Desde costeo app", tone="red" if utilidad_total_real_con_extra < 0 else "green")
            with c2:
                kpi("Utilidad neta contable", money(utilidad_contable), help_text="Desde P&G contable", tone="red" if utilidad_contable < 0 else "green")
            with c3:
                kpi("Diferencia a conciliar", money(diferencia_utilidad), help_text="Contabilidad - app", tone="yellow" if abs(diferencia_utilidad) > 1 else "green")
            with c4:
                estado = "CONCILIADA" if abs(diferencia_utilidad) < 1 else "REQUIERE PUENTE"
                kpi("Estado", estado, help_text="Usa el puente inferior", tone="green" if estado == "CONCILIADA" else "yellow")

            st.markdown("### Estado de resultados contable reconstruido")
            dataframe_gerencial(pyg["resumen"])

            st.markdown("### Puente ejecutivo app → utilidad neta contable")
            puente = pd.DataFrame([
                ["Utilidad gerencial con extraordinarios", utilidad_total_real_con_extra, "Resultado app después de gastos extraordinarios"],
                ["Diferencia ingresos netos contables vs venta app", diferencia_ventas, "Precio promedio, devoluciones, redondeos o ventas no mapeadas por unidad"],
                ["Diferencia neta en costos/gastos/financieros/impuestos", diferencia_restante, "Todo lo que el P&G mide diferente al costeo gerencial"],
                ["Utilidad neta contable conciliada", utilidad_total_real_con_extra + diferencia_ventas + diferencia_restante, "Debe igualar utilidad neta contable"],
                ["Check", (utilidad_total_real_con_extra + diferencia_ventas + diferencia_restante) - utilidad_contable, "Debe ser cero"],
            ], columns=["Paso", "Valor", "Lectura"])
            dataframe_gerencial(puente)

            st.markdown("### Partidas extraordinarias: validar doble castigo")
            extra_dups = analizar_extraordinarios_duplicados(df_mes)
            if extra_dups.empty:
                st.success("No hay gastos extraordinarios cargados para el periodo seleccionado.")
            else:
                st.warning("Si una observación está en gastos recurrentes y también en extraordinarios, no debe castigarse dos veces. Reclasifica o excluye antes de presentar utilidad empresa.")
                dataframe_gerencial(extra_dups)

            st.markdown("### Candidatos cercanos a la diferencia")
            cand = candidatos_cercanos_contabilidad(contabilidad_df, periodo, diferencia_utilidad, tolerancia_pct=0.15)
            if cand.empty:
                st.info("No encontré cuentas individuales dentro de ±15% de la diferencia. Revise el puente por agrupaciones.")
            else:
                dataframe_gerencial(cand.head(20))
                st.caption("Las cuentas de balance cercanas sirven como control de asiento, no como gasto. Ejemplo típico: provisión de intereses contra gasto financiero.")

            st.markdown("### Detalle P&G por auxiliar")
            detalle_cols = ["Auxiliar", "NombreAux", "TotalDebito", "TotalCredito", "Saldo", "Movimiento P&G"]
            dataframe_gerencial(pyg["detalle"][[c for c in detalle_cols if c in pyg["detalle"].columns]].head(80))

            with st.expander("Control de balance: cuentas 1/2/3 no deben entrar al costeo"):
                bal = pyg["balance_control"]
                if bal.empty:
                    st.info("No hay cuentas de balance en el periodo contable cargado.")
                else:
                    control_cols = ["Auxiliar", "NombreAux", "TotalDebito", "TotalCredito", "Saldo", "Valor absoluto mayor"]
                    dataframe_gerencial(bal[[c for c in control_cols if c in bal.columns]].head(80))

            st.markdown("### Dictamen CFO")
            if abs(diferencia_utilidad) < 1:
                st.success("La utilidad gerencial con extraordinarios concilia contra utilidad neta contable.")
            else:
                st.error(
                    "No presentes una sola métrica llamada 'utilidad empresa'. Presenta el puente: recurrente, extraordinarios y neta contable. "
                    "Ese es el gobierno correcto del número."
                )
