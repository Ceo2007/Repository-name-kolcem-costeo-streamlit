...
st.set_page_config(
    page_title="Kolcem · P&G Contable y Costeo",
    page_icon="🏭",
    layout="wide",
...
@st.cache_data(show_spinner=False)
def load_pyg_oficial(uploaded_bytes: bytes) -> pd.DataFrame:
    """Carga el P&G oficial exportado en formato de estado de resultados.

    Formato esperado: columnas Código / Cuenta / Periodo, aunque el encabezado no
    esté en la primera fila. El valor de Periodo se deja con el signo original del
    reporte: ingresos normalmente negativos, gastos/costos positivos y utilidad
    del ejercicio negativa cuando hay ganancia.
    """
    xls = pd.ExcelFile(io.BytesIO(uploaded_bytes), engine="openpyxl")
    frames = []
    for sheet in xls.sheet_names:
        ...
        for i in range(min(len(raw), 25)):
            ...
            if "CODIGO" in vals and "CUENTA" in vals and "PERIODO" in vals:
                header_idx = i
                break
        if header_idx is None:
            continue
        df = pd.read_excel(xls, sheet_name=sheet, header=header_idx, engine="openpyxl")
        ren = {}
        for c in df.columns:
            nc = norm_text(c)
            if nc in {"CODIGO", "CÓDIGO"}:
                ren[c] = "Codigo"
            elif nc == "CUENTA":
                ren[c] = "Cuenta"
            elif nc == "PERIODO":
        ...
        if {"Codigo", "Cuenta", "Periodo"}.issubset(df.columns):
            df = df[["Codigo", "Cuenta", "Periodo"]].copy()
            df["Hoja"] = sheet
            frames.append(df)
    if not frames:
        raise ValueError("El P&G oficial debe tener columnas Código, Cuenta y Periodo.")

    out = pd.concat(frames, ignore_index=True).dropna(how="all").copy()
    out["Cuenta"] = out["Cuenta"].fillna("").astype(str).str.strip()
    out["Cuenta_norm"] = out["Cuenta"].map(norm_text)
    out["Periodo"] = out["Periodo"].apply(parse_valor).astype(float)
    out["Codigo"] = out["Codigo"].apply(lambda x: "" if pd.isna(x) else str(x).replace(".0", "").strip())
    out["Codigo"] = out["Codigo"].str.extract(r"(\d+)", expand=False).fillna("")
    out["Clase"] = out["Codigo"].str[0]
    return out


def pyg_oficial_summary(pyg_oficial: pd.DataFrame | None) -> dict:
    if pyg_oficial is None or pyg_oficial.empty:
        return {"ok": False, "error": "No hay P&G oficial cargado."}
    d = pyg_oficial.copy()
    utilidad_rows = d[d["Cuenta_norm"].str.contains("UTILIDAD DEL EJERCICIO", na=False)].copy()
    utilidad = -float(utilidad_rows["Periodo"].iloc[-1]) if not utilidad_rows.empty else np.nan
    ingresos = -float(d.loc[d["Codigo"].str.startswith("41", na=False), "Periodo"].sum())
    otros_ingresos = -float(d.loc[d["Codigo"].str.startswith("42", na=False), "Periodo"].sum())
    gastos = float(d.loc[d["Clase"].isin(["5"]), "Periodo"].sum())
    costo_venta = float(d.loc[d["Codigo"].str.startswith("61", na=False), "Periodo"].sum())
    costos_produccion = float(d.loc[d["Clase"].isin(["7"]), "Periodo"].sum())
    return {
        "ok": True,
        "df": d,
        "Ingresos operacionales PYG": ingresos,
        "Otros ingresos PYG": otros_ingresos,
        "Gastos PYG": gastos,
        "Costo venta PYG": costo_venta,
        "Costos producción PYG": costos_produccion,
        "Utilidad neta PYG archivo": utilidad,
        "Margen neto PYG archivo": safe_div(utilidad, ingresos) if ingresos > 0 else np.nan,
    }


def conciliar_pyg_oficial_vs_contabilidad(
    pyg_oficial: pd.DataFrame | None,
    cont: pd.DataFrame | None,
    ano: int,
    mes: int,
    tolerancia: float = 5000.0,
) -> dict:
    """Cruza el P&G oficial por código de 4 dígitos contra la contabilidad auxiliar.

    El P&G oficial viene agrupado por código mayor (4120, 4210, 5305, 6120, etc.).
    La contabilidad viene por auxiliar largo. La conciliación compara cada código
    del P&G contra la suma de auxiliares que empiezan por ese código.
    """
    if pyg_oficial is None or pyg_oficial.empty:
        return {"ok": False, "error": "No hay P&G oficial cargado.", "conciliacion": pd.DataFrame(), "resumen": {}}
    if cont is None or cont.empty:
        return {"ok": False, "error": "No hay contabilidad cargada.", "conciliacion": pd.DataFrame(), "resumen": {}}

    pyg_det = pyg_oficial[pyg_oficial["Codigo"] != ""].copy()
    pyg_grp = (
        pyg_det.groupby(["Codigo", "Cuenta"], as_index=False)["Periodo"].sum()
        .rename(columns={"Periodo": "Valor PYG oficial"})
    )
    cont_mes = cont[(cont["Ano"] == ano) & (cont["Mes"] == mes)].copy()
    rows = []
    for _, r in pyg_grp.iterrows():
        codigo = str(r["Codigo"])
        saldo_cont = float(cont_mes.loc[cont_mes["Auxiliar"].astype(str).str.startswith(codigo, na=False), "Saldo"].sum())
        valor_pyg = float(r["Valor PYG oficial"])
        diferencia = valor_pyg - saldo_cont
        rows.append({
            "Estado": "OK" if abs(diferencia) <= tolerancia else "DIFERENCIA",
            "Codigo": codigo,
            "Cuenta PYG": r["Cuenta"],
            "Valor PYG oficial": valor_pyg,
            "Saldo contabilidad": saldo_cont,
            "Diferencia PYG - Contabilidad": diferencia,
            "% diferencia": safe_div(diferencia, abs(saldo_cont)) if abs(saldo_cont) > 0 else np.nan,
        })
    conc = pd.DataFrame(rows)
    if not conc.empty:
        conc["Diferencia abs"] = conc["Diferencia PYG - Contabilidad"].abs()
        conc["_orden"] = conc["Estado"].map({"DIFERENCIA": 0, "OK": 1}).fillna(9)
        conc = conc.sort_values(["_orden", "Diferencia abs"], ascending=[True, False]).drop(columns=["_orden"])

    util_pyg = pyg_oficial_summary(pyg_oficial).get("Utilidad neta PYG archivo", np.nan)
    pyg_cont = pyg_summary(cont, ano, mes)
    util_cont = pyg_cont.get("Utilidad neta", np.nan) if pyg_cont.get("ok") else np.nan
    dif_util = util_pyg - util_cont if not pd.isna(util_pyg) and not pd.isna(util_cont) else np.nan
    resumen = {
        "Utilidad PYG archivo": util_pyg,
        "Utilidad contabilidad": util_cont,
        "Diferencia utilidad": dif_util,
        "Códigos PYG": int(conc["Codigo"].nunique()) if not conc.empty else 0,
        "Códigos OK": int((conc["Estado"] == "OK").sum()) if not conc.empty else 0,
        "Códigos con diferencia": int((conc["Estado"] == "DIFERENCIA").sum()) if not conc.empty else 0,
        "Diferencia absoluta total": float(conc["Diferencia PYG - Contabilidad"].abs().sum()) if not conc.empty else 0.0,
    }
    return {"ok": True, "conciliacion": conc, "resumen": resumen, "pyg_contable": pyg_cont}
...
SUBTOTALES_COSTEO_EXCLUIR = {
    norm_text("MO"),
    norm_text("CIF"),
    norm_text("COSTOS A REPARTIR"),
    norm_text("COSTOS MATERIA PRIMA"),
    norm_text("MATERIA PRIMA Q"),
...
def linea_costeo_incluida_mask(df: pd.DataFrame) -> pd.Series:
    """Evita doble conteo de subtotales escritos dentro de índices de costo.

    Hallazgo crítico: en ART empacado existe una fila con Observacion = CIF
    marcada como C MO ART EMP. Si se suma C MO ART EMP + C CIF ART EMP,
    esa fila duplica el CIF. Esta máscara excluye esos subtotales visibles
    sin tocar las líneas contables reales.
    """
    return ~df["Observacion_norm"].isin(SUBTOTALES_COSTEO_EXCLUIR)


def sum_indices(df: pd.DataFrame, produccion: str, indices: Iterable[str]) -> float:
    idx = {norm_text(i) for i in indices}
    base = df[(df["Produccion_norm"] == norm_text(produccion)) & (df["Indice_norm"].isin(idx))].copy()
    base = base[linea_costeo_incluida_mask(base)].copy()
...
def gastos_asignados(df: pd.DataFrame, producto: Vendible, incluir_renta: bool, incluir_patrimonio: bool) -> float:
    ...
    # ART se calcula desde el TOTAL/GASTOS unitario ya preparado en la base.
    # Antes esta rama retornaba inmediatamente y por eso los checkboxes de
    # renta/patrimonio no afectaban ART. Ahora conserva el criterio original
    # de TOTAL/GASTOS, pero suma los impuestos solo cuando el usuario los activa.
    if producto.key == "ART_42_5":
        total = float(
            base.loc[
                (base["Indice_norm"] == norm_text("TOTAL"))
                & (base["Observacion_norm"] == norm_text("GASTOS")),
                "Valor",
            ].sum()
        )
        if incluir_renta:
            total += float(base.loc[base["Indice_norm"] == norm_text("C IMP REN"), "Valor"].sum())
        if incluir_patrimonio:
            total += float(base.loc[base["Indice_norm"] == norm_text("C IMP PATR"), "Valor"].sum())
        return total
...
def detalle_lineas_indices(df_mes: pd.DataFrame, produccion: str, indices: Iterable[str]) -> pd.DataFrame:
    idx = {norm_text(i) for i in indices}
    d = df_mes[(df_mes["Produccion_norm"] == norm_text(produccion)) & (df_mes["Indice_norm"].isin(idx))].copy()
    if d.empty:
        return pd.DataFrame(columns=["Produccion", "Indice", "Observacion", "Valor", "Incluido", "Motivo"])
    d["Incluido"] = linea_costeo_incluida_mask(d)
    d["Motivo"] = np.where(d["Incluido"], "Línea contable sumada", "Subtotal excluido para evitar doble conteo")
    return d[["Produccion", "Indice", "Observacion", "Valor", "Incluido", "Motivo"]].copy()


def detalle_gastos_producto(df_mes: pd.DataFrame, producto: Vendible, incluir_renta: bool, incluir_patrimonio: bool) -> pd.DataFrame:
    base = df_mes[df_mes["Produccion_norm"] == norm_text(producto.prod_gastos)].copy()
    if base.empty:
        return pd.DataFrame(columns=["Produccion", "Indice", "Observacion", "Valor", "Incluido", "Motivo"])

    if producto.key == "ART_42_5":
        rows = base[
            ((base["Indice_norm"] == norm_text("TOTAL")) & (base["Observacion_norm"] == norm_text("GASTOS")))
            | ((base["Indice_norm"] == norm_text("C IMP REN")) & incluir_renta)
            | ((base["Indice_norm"] == norm_text("C IMP PATR")) & incluir_patrimonio)
        ].copy()
        rows["Incluido"] = True
        rows["Motivo"] = np.where(
            rows["Indice_norm"] == norm_text("TOTAL"),
            "Gastos ART base: TOTAL / GASTOS",
            "Impuesto incluido por checkbox",
        )
        return rows[["Produccion", "Indice", "Observacion", "Valor", "Incluido", "Motivo"]].copy()
    ...
    if incluir_patrimonio:
        idx.add(norm_text("C IMP PATR"))
    rows = base[base["Indice_norm"].isin(idx)].copy()
    rows["Incluido"] = ~fx_no_realizada_mask(rows)
    rows["Motivo"] = np.where(rows["Incluido"], "Gasto asignado sumado", "FX no realizada excluida")
    return rows[["Produccion", "Indice", "Observacion", "Valor", "Incluido", "Motivo"]].copy()


def auditoria_costeo(df_mes: pd.DataFrame, incluir_renta: bool, incluir_patrimonio: bool) -> dict:
    resumen_rows = []
    detalle_rows = []

    for cfg in FISICOS:
        producto_nombre = cfg["Producto"]
        cantidad = sum_obs(df_mes, cfg["Produccion"], cfg["Obs cantidad"])
        peso = cfg["Presentación kg"]
        if cfg["Unidad"] == "kg":
            kg = cantidad
        ...
        det_ind = detalle_lineas_indices(df_mes, cfg["Produccion"], cfg["Indices costo"])
        costo_ind = float(det_ind.loc[det_ind["Incluido"], "Valor"].sum()) if not det_ind.empty else 0.0
        if not det_ind.empty:
            det_ind["Producto costeo"] = producto_nombre
            det_ind["Bloque"] = "Costo industrial"
            detalle_rows.append(det_ind)

        gastos = 0.0
        formula_gastos = "No aplica"
        if cfg["Vendible key"]:
            prod_v = VENDIBLES[cfg["Vendible key"]]
            det_g = detalle_gastos_producto(df_mes, prod_v, incluir_renta, incluir_patrimonio)
            gastos = float(det_g.loc[det_g["Incluido"], "Valor"].sum()) if not det_g.empty else 0.0
            formula_gastos = (
                "ART: TOTAL/GASTOS + C IMP REN/C IMP PATR si checkbox"
                if prod_v.key == "ART_42_5"
                else "UG: índices gasto configurados + impuestos si checkbox; excluye FX no realizada"
            )
            if not det_g.empty:
                det_g["Producto costeo"] = producto_nombre
                det_g["Bloque"] = "Gastos asignados"
                detalle_rows.append(det_g)

        costo_total = costo_ind + gastos
        costo_kg = safe_div(costo_total, kg)
        costo_bolsa = costo_kg * float(peso) if cfg["Unidad"] == "bolsa" and not pd.isna(peso) else np.nan

        formula_costo = (
            "Costo total = SUMA(índices industriales incluidos) + gastos asignados; "
            "Costo/kg = Costo total / kg producidos; Costo/bolsa = Costo/kg x presentación"
        )
        resumen_rows.append({
            "Producto": producto_nombre,
            "Producción base": cfg["Produccion"],
            "Cantidad base": cantidad,
            "Unidad cantidad": cfg["Unidad"],
            "Kg producidos": kg,
            "Bolsas producidas": bolsas,
            "Índices industriales": " | ".join(cfg["Indices costo"]),
            "Fórmula industrial": "SUMA líneas incluidas de índices industriales, excluyendo subtotales MO/CIF para evitar doble conteo",
            "Fórmula gastos": formula_gastos,
            "Costo industrial": costo_ind,
            "Gastos asignados": gastos,
            "Costo total costeo": costo_total,
            "Costo / kg": costo_kg,
            "Costo / bolsa": costo_bolsa,
            "Fórmula general": formula_costo,
        })

    detalle = pd.concat(detalle_rows, ignore_index=True) if detalle_rows else pd.DataFrame()
    if not detalle.empty:
        detalle["Valor incluido"] = np.where(detalle["Incluido"], detalle["Valor"], 0.0)
        consolidado = (
            detalle.groupby(["Producto costeo", "Bloque", "Produccion", "Indice"], as_index=False)
            .agg(
                Valor_total=("Valor", "sum"),
                Valor_incluido=("Valor incluido", "sum"),
                Lineas=("Valor", "size"),
                Lineas_incluidas=("Incluido", "sum"),
                Motivos=("Motivo", lambda s: join_unicos(s, 4)),
            )
            .sort_values(["Producto costeo", "Bloque", "Indice"])
        )
    else:
        consolidado = pd.DataFrame()
    return {"resumen": pd.DataFrame(resumen_rows), "detalle": detalle, "consolidado": consolidado}
...
def pyg_summary(cont: pd.DataFrame | None, ano: int, mes: int) -> dict:
    ...
    # sin explicar eficiencia operativa. Regla corregida: solo cuentas P&G. No mezclar
    # pasivos/activos de balance que también puedan llamarse diferencia en cambio.
    nombre_norm = d["NombreAux"].map(norm_text) if "NombreAux" in d.columns else pd.Series("", index=d.index)
    aux = d["Auxiliar"].astype(str).str.strip()
    mask_fx_ingreso = aux.str.startswith("421020", na=False) | (aux.str.startswith("42", na=False) & nombre_norm.str.contains("DIFERENCIA EN CAMBIO", na=False))
    mask_fx_gasto = aux.str.startswith("530525", na=False) | (aux.str.startswith("53", na=False) & nombre_norm.str.contains("DIFERENCIA EN CAMBIO", na=False))
    mask_fx_pyg = mask_fx_ingreso | mask_fx_gasto
    mask_fx_no_realizada = mask_fx_pyg & nombre_norm.str.contains("NO REALIZ", na=False)
    mask_fx_realizada = mask_fx_pyg & ~mask_fx_no_realizada

    fx_ingresos = -float(d.loc[mask_fx_ingreso, "Saldo"].sum())
    fx_gastos = float(d.loc[mask_fx_gasto, "Saldo"].sum())
    fx_neta = fx_ingresos - fx_gastos
    fx_realizada = (
        -float(d.loc[mask_fx_realizada & mask_fx_ingreso, "Saldo"].sum())
        - float(d.loc[mask_fx_realizada & mask_fx_gasto, "Saldo"].sum())
    )
    fx_no_realizada = (
        -float(d.loc[mask_fx_no_realizada & mask_fx_ingreso, "Saldo"].sum())
        - float(d.loc[mask_fx_no_realizada & mask_fx_gasto, "Saldo"].sum())
    )
    otros_sin_fx = otros - fx_ingresos
    financieros_sin_fx = financieros - fx_gastos
    ...
    if abs(fx_neta) > 1:
        alertas_pyg.append(
            f"La utilidad incluye diferencia en cambio neta P&G por {money(fx_neta)} "
            f"(ingresos {money(fx_ingresos)} menos gastos {money(fx_gastos)})."
        )
    ...
    utilidad_sin_fx_neta = utilidad - fx_neta
    utilidad_sin_fx_no_realizada = utilidad - fx_no_realizada
    utilidad_sin_clase42 = utilidad - otros
    utilidad_sin_clase42_y_sin_fx_no_realizada = utilidad - otros - fx_no_realizada
    ...
    return {
        ...
        "Diferencia en cambio ingresos": fx_ingresos,
        "Diferencia en cambio gastos": fx_gastos,
        "Diferencia en cambio neta": fx_neta,
        ...
        "Gastos financieros sin diferencia en cambio": financieros_sin_fx,
        "Utilidad sin diferencia en cambio neta": utilidad_sin_fx_neta,
        "Utilidad sin diferencia en cambio no realizada": utilidad_sin_fx_no_realizada,
        "Utilidad sin clase 42": utilidad_sin_clase42,
        "Utilidad sin clase 42 y sin FX no realizada": utilidad_sin_clase42_y_sin_fx_no_realizada,
...
def executive_ai(metrics: pd.DataFrame, fis: pd.DataFrame, hist: pd.DataFrame, pyg: dict, margen_meta: float, conf: float, warn: list[str]) -> pd.DataFrame:
    ...
    if pyg.get("ok") and pyg.get("P&G completo", True):
        ...
        rows.append({
            ...
            "Hallazgo": f"Utilidad neta contable P&G: {money(net)}; margen neto {pct(mg)}. Separar de resultado operativo y diferencia en cambio.",
            "Impacto": net,
...
with st.sidebar:
    ...
    cont_file = st.file_uploader("Contabilidad detallada por auxiliar", type=["xlsx", "xlsm"], key="cont")
    pyg_file = st.file_uploader("P&G oficial · Estado de resultados", type=["xlsx", "xlsm"], key="pyg_oficial")
    st.markdown("### Periodo")
...
st.markdown('<div class="main-title">Kolcem · P&G Contable y Costeo por Producto</div>', unsafe_allow_html=True)
st.markdown('<div class="subtitle">Primero utilidad operativa de planta; después P&G contable, costeo por kg, precio defendible, conciliación y alertas. Sin ruido visual.</div>', unsafe_allow_html=True)
...
pyg_oficial = None
pyg_archivo = {"ok": False, "error": "No hay P&G oficial cargado."}
pyg_conc = {"ok": False, "error": "Carga P&G oficial y contabilidad para cruzarlos.", "conciliacion": pd.DataFrame(), "resumen": {}}
if pyg_file:
    try:
        pyg_oficial = load_pyg_oficial(pyg_file.getvalue())
        pyg_archivo = pyg_oficial_summary(pyg_oficial)
        if cont is not None:
            pyg_conc = conciliar_pyg_oficial_vs_contabilidad(pyg_oficial, cont, int(ano), int(mes))
    except Exception as e:
        pyg_archivo = {"ok": False, "error": str(e)}
        pyg_conc = {"ok": False, "error": str(e), "conciliacion": pd.DataFrame(), "resumen": {}}
...
with tabs[0]:
    section("Utilidad operativa de la planta cementera")
    st.markdown(
        "<div class='executive-strip'><b>Esta es la pantalla que debe mandar la conversación operativa:</b> "
        "ventas de cemento menos costo de venta, menos administración y ventas/logística. "
        "No incluye diferencia en cambio, otros ingresos, intereses, impuestos ni maniobras financieras. "
        "Eso va en P&G financiero, no en desempeño de planta.</div>",
        unsafe_allow_html=True,
    )

    if pyg.get("ok") and pyg.get("P&G completo", True):
        ingresos_planta = float(pyg.get("Ingresos netos", 0))
        costo_venta_planta = float(pyg.get("Costo venta", 0))
        utilidad_bruta_planta = float(pyg.get("Ganancia bruta", 0))
        gastos_admin_planta = float(pyg.get("Gastos administración", 0))
        gastos_venta_planta = float(pyg.get("Gastos venta", 0))
        utilidad_operativa_planta = float(pyg.get("Resultado operativo", 0))
        margen_bruto_planta = safe_div(utilidad_bruta_planta, ingresos_planta)
        margen_operativo_planta = safe_div(utilidad_operativa_planta, ingresos_planta)
        fx_neta_excluida = float(pyg.get("Diferencia en cambio neta", 0))
        otros_clase42 = float(pyg.get("Otros ingresos / financieros clase 42", 0))
        financieros_total = float(pyg.get("Gastos financieros", 0))
        impuestos_total = float(pyg.get("Impuestos", 0))
        utilidad_neta_pyg = float(pyg.get("Utilidad neta", 0))

        p1, p2, p3, p4, p5 = st.columns(5)
        with p1:
            card("Utilidad planta", money(utilidad_operativa_planta), "EBIT operativo cementero", "good" if utilidad_operativa_planta >= 0 else "bad")
        with p2:
            card("Margen planta", pct(margen_operativo_planta), "Utilidad planta / ingresos cemento", "good" if margen_operativo_planta >= margen_meta else "warn")
        with p3:
            card("Ingresos cemento", money(ingresos_planta), "Solo operación cementera", "blue")
        with p4:
            card("Ganancia bruta", money(utilidad_bruta_planta), f"Margen {pct(margen_bruto_planta)}", "good" if utilidad_bruta_planta >= 0 else "bad")
        with p5:
            card("FX excluida", money(fx_neta_excluida), "No es desempeño de planta", "warn" if abs(fx_neta_excluida) > 1 else "good")

        section("Estado operativo de planta")
        planta_df = pd.DataFrame([
            {"Línea": "Ingresos operacionales cemento", "Valor": ingresos_planta, "Margen sobre ingresos": 1.0, "Lectura": "Venta neta de cemento; base de la planta"},
            {"Línea": "Costo de venta cemento", "Valor": -costo_venta_planta, "Margen sobre ingresos": -safe_div(costo_venta_planta, ingresos_planta), "Lectura": "Costo reconocido del producto vendido"},
            {"Línea": "Utilidad bruta planta", "Valor": utilidad_bruta_planta, "Margen sobre ingresos": margen_bruto_planta, "Lectura": "Potencia industrial antes de estructura"},
            {"Línea": "Gastos administración", "Valor": -gastos_admin_planta, "Margen sobre ingresos": -safe_div(gastos_admin_planta, ingresos_planta), "Lectura": "Estructura administrativa que debe absorber la planta"},
            {"Línea": "Gastos venta y logística", "Valor": -gastos_venta_planta, "Margen sobre ingresos": -safe_div(gastos_venta_planta, ingresos_planta), "Lectura": "Comercial, fletes, coordinación logística y distribución"},
            {"Línea": "UTILIDAD OPERATIVA PLANTA", "Valor": utilidad_operativa_planta, "Margen sobre ingresos": margen_operativo_planta, "Lectura": "Número principal para gerenciar la planta cementera"},
        ])
        st.dataframe(fmt_df(planta_df), use_container_width=True, hide_index=True)

        section("Puente: planta vs utilidad financiera")
        puente_planta = pd.DataFrame([
            {"Paso": "Utilidad operativa planta", "Impacto": utilidad_operativa_planta, "Se incluye en planta": "Sí", "Lectura": "Resultado de vender y operar cemento"},
            {"Paso": "Otros ingresos / financieros clase 42", "Impacto": otros_clase42, "Se incluye en planta": "No", "Lectura": "No es eficiencia industrial"},
            {"Paso": "Diferencia en cambio neta dentro de clase 42/53", "Impacto": fx_neta_excluida, "Se incluye en planta": "No", "Lectura": "Se muestra para no confundir caja/FX con operación"},
            {"Paso": "Gastos financieros", "Impacto": -financieros_total, "Se incluye en planta": "No", "Lectura": "Costo financiero, no desempeño operativo"},
            {"Paso": "Impuestos", "Impacto": -impuestos_total, "Se incluye en planta": "No", "Lectura": "Resultado fiscal/contable"},
            {"Paso": "Utilidad neta P&G", "Impacto": utilidad_neta_pyg, "Se incluye en planta": "No como KPI de planta", "Lectura": "Resultado contable final"},
        ])
        st.dataframe(fmt_df(puente_planta), use_container_width=True, hide_index=True)

        section("Lectura por producto: sirve para precio, no para reemplazar el P&G")
        producto_cols = [
            "Producto", "Bolsas vendidas", "Ton vendidas", "Venta", "Costo vendido recurrente",
            "Utilidad recurrente", "Margen recurrente", "Precio bolsa", "Costo bolsa recurrente", "Costo kg recurrente",
        ]
        producto_cols = [c for c in producto_cols if c in metrics.columns]
        st.dataframe(fmt_df(metrics[producto_cols]), use_container_width=True, hide_index=True)

        section("Regla de gobierno")
        st.markdown(
            "<div class='decision-box'>"
            "<b>Regla brutalmente simple:</b><br>"
            "1. Para saber si la planta gana plata: mirar <b>Utilidad operativa planta</b>.<br>"
            "2. Para saber qué producto destruye o crea margen: mirar <b>costeo por producto</b>.<br>"
            "3. Para saber utilidad legal/fiscal: mirar <b>P&G neto</b>.<br>"
            "4. Para decidir precio: usar <b>precio defendible</b>, no la utilidad financiera inflada por FX."
            "</div>",
            unsafe_allow_html=True,
        )
    elif pyg.get("ok") and not pyg.get("P&G completo", True):
        st.error("No se puede calcular utilidad de planta confiable porque el P&G cargado está incompleto.")
        for alerta in pyg.get("Alertas P&G", []):
            st.warning(alerta)
        st.dataframe(fmt_df(metrics), use_container_width=True, hide_index=True)
    else:
        st.warning("Carga la contabilidad/P&G para ver la utilidad operativa de planta. Sin P&G, solo queda el modelo gerencial de costeo.")
        fallback_df = pd.DataFrame([
            {"Línea": "Venta app", "Valor": bridge.get("venta_app", 0), "Lectura": "Venta calculada por unidades y precios de la base"},
            {"Línea": "Costo vendido recurrente app", "Valor": -bridge.get("costo_vendido", 0), "Lectura": "Costo gerencial, no P&G oficial"},
            {"Línea": "Utilidad recurrente app", "Valor": bridge.get("utilidad_recurrente", 0), "Lectura": "Sirve para precio/producto; no es utilidad oficial de planta"},
        ])
        st.dataframe(fmt_df(fallback_df), use_container_width=True, hide_index=True)
...
with tabs[1]:
    ...
    if pyg.get("ok") and pyg_completo:
        st.markdown(
            f"<div class='executive-strip'><b>Utilidad neta contable P&G:</b> {money(pyg['Utilidad neta'])} en {mes_nombre} {ano}. "
            f"<b>Resultado operativo:</b> {money(pyg['Resultado operativo'])}. "
            "La utilidad neta incluye partidas operativas y no operativas; no equivale a flujo de caja ni a eficiencia de planta.</div>",
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            f"<div class='executive-strip'><b>No puedo mostrar P&G contable oficial:</b> {pyg.get('error', 'falta contabilidad')}. "
            "La app seguirá mostrando costeo gerencial, pero eso no es utilidad neta contable de empresa.</div>",
            unsafe_allow_html=True,
        )
    ...
    if pyg.get("ok") and pyg_completo:
        with c1:
            card("Utilidad neta P&G", money(pyg["Utilidad neta"]), "Incluye clase 42 / FX", "good" if pyg["Utilidad neta"] >= 0 else "bad")
        with c2:
        ...
        with c1:
            card("Utilidad neta P&G", "N/A", "Cargue P&G", "warn")
        with c2:
    ...
    if pyg.get("ok"):
        linea_final = "UTILIDAD NETA CONTABLE P&G" if pyg_completo else "RESULTADO CONTABLE PARCIAL - NO USAR COMO UTILIDAD REAL"
        pyg_table = pd.DataFrame([
            ...
            {"Línea": "Diferencia en cambio ingresos", "Valor": pyg.get("Diferencia en cambio ingresos", 0), "Lectura": "Solo cuentas P&G clase 42; no incluye balance"},
            {"Línea": "Diferencia en cambio gastos", "Valor": -pyg.get("Diferencia en cambio gastos", 0), "Lectura": "Solo cuentas P&G clase 53; no incluye balance"},
            {"Línea": "Diferencia en cambio neta", "Valor": pyg.get("Diferencia en cambio neta", 0), "Lectura": "Efecto neto FX sobre utilidad"},
            {"Línea": "Diferencia en cambio realizada neta", "Valor": pyg.get("Diferencia en cambio realizada", 0), "Lectura": "Realizada, neta entre ingreso y gasto"},
            {"Línea": "Diferencia en cambio no realizada neta", "Valor": pyg.get("Diferencia en cambio no realizada", 0), "Lectura": "No realizada; puede inflar utilidad sin caja"},
            {"Línea": "Otros ingresos sin diferencia en cambio", "Valor": pyg.get("Otros ingresos sin diferencia en cambio", 0), "Lectura": "Clase 42 depurada de FX ingreso"},
            {"Línea": "Gastos financieros sin diferencia en cambio", "Valor": -pyg.get("Gastos financieros sin diferencia en cambio", pyg.get("Gastos financieros", 0)), "Lectura": "Clase 53 depurada de FX gasto"},
            {"Línea": "Utilidad sin diferencia en cambio neta", "Valor": pyg.get("Utilidad sin diferencia en cambio neta", 0), "Lectura": "Lectura operativa conservadora: elimina FX neta"},
            {"Línea": "Utilidad sin diferencia en cambio no realizada", "Valor": pyg.get("Utilidad sin diferencia en cambio no realizada", 0), "Lectura": "Elimina únicamente FX no realizada"},
            {"Línea": "Utilidad sin clase 42", "Valor": pyg.get("Utilidad sin clase 42", 0), "Lectura": "Lectura operacional ampliada: excluye otros ingresos / financieros"},
            ...
            {"Línea": linea_final, "Valor": pyg["Utilidad neta"], "Lectura": "Resultado neto contable; debe leerse junto con resultado operativo, clase 42 y diferencia en cambio"},
        ])
        st.dataframe(fmt_df(pyg_table), use_container_width=True, hide_index=True)
    else:
        st.warning(pyg.get("error", "No hay P&G legible."))

    section("P&G oficial vs contabilidad")
    if pyg_archivo.get("ok") and pyg_conc.get("ok"):
        res_pyg = pyg_conc.get("resumen", {})
        pc1, pc2, pc3, pc4 = st.columns(4)
        with pc1:
            card("Utilidad P&G archivo", money(res_pyg.get("Utilidad PYG archivo", 0)), "Estado de resultados cargado", "blue")
        with pc2:
            card("Utilidad contabilidad", money(res_pyg.get("Utilidad contabilidad", 0)), "Calculada desde auxiliares", "blue")
        with pc3:
            dif_util = float(res_pyg.get("Diferencia utilidad", 0) or 0)
            card("Diferencia utilidad", money(dif_util), "Debe ser $0", "good" if abs(dif_util) <= 5000 else "bad")
        with pc4:
            dif_cod = int(res_pyg.get("Códigos con diferencia", 0) or 0)
            card("Códigos con diferencia", str(dif_cod), "PYG mayor vs auxiliares", "good" if dif_cod == 0 else "warn")

        conc_pyg_view = pyg_conc["conciliacion"].copy()
        st.dataframe(fmt_df(conc_pyg_view), use_container_width=True, hide_index=True)
    elif pyg_file and not pyg_conc.get("ok"):
        st.warning(pyg_conc.get("error", pyg_archivo.get("error", "No se pudo conciliar el P&G oficial.")))
    else:
        st.info("Carga el P&G oficial separado para validar que el estado de resultados cuadre contra la contabilidad auxiliar.")
...
Utilidad neta contable P&G: {money(pyg.get('Utilidad neta', np.nan)) if pyg.get('ok') and pyg.get('P&G completo', True) else 'N/A - P&G incompleto o no cargado'}.
...
with tabs[8]:
    section("Fórmula auditable de cada costeo")
    st.markdown(
        "<div class='executive-strip'><b>Control anticaja negra:</b> aquí se ve qué se suma, qué se excluye y cómo se llega al costo/kg y costo/bolsa. "
        "La regla nueva excluye subtotales visibles como MO/CIF cuando aparecen dentro de un índice para evitar doble conteo. "
        "En ART empacado esto corrige el CIF duplicado que venía escondido dentro de C MO ART EMP.</div>",
        unsafe_allow_html=True,
    )

    audit = auditoria_costeo(df_mes, incluir_renta, incluir_patrimonio)
    resumen_audit = audit["resumen"]
    consolidado_audit = audit["consolidado"]
    detalle_audit = audit["detalle"]

    section("Resumen de fórmulas por producto")
    cols_res = [
        "Producto", "Producción base", "Cantidad base", "Unidad cantidad", "Kg producidos", "Bolsas producidas",
        "Índices industriales", "Fórmula industrial", "Fórmula gastos", "Costo industrial", "Gastos asignados",
        "Costo total costeo", "Costo / kg", "Costo / bolsa", "Fórmula general",
    ]
    st.dataframe(fmt_df(resumen_audit[cols_res]), use_container_width=True, hide_index=True)

    section("Validación específica ART")
    art_row = resumen_audit[resumen_audit["Producto"] == "ART empacado 42,5 kg"].copy()
    if not art_row.empty:
        ar = art_row.iloc[0]
        aa1, aa2, aa3, aa4 = st.columns(4)
        with aa1:
            card("ART costo / bolsa", money(ar["Costo / bolsa"]), "Después de excluir CIF duplicado", "blue")
        with aa2:
            card("ART costo / kg", money(ar["Costo / kg"], 2), "Costo total / kg", "blue")
        with aa3:
            card("ART industrial", money(ar["Costo industrial"]), "MP + MO + CIF sin subtotal duplicado", "neutral")
        with aa4:
            card("ART gastos", money(ar["Gastos asignados"]), "Incluye renta/patrimonio si checkbox", "warn" if incluir_renta or incluir_patrimonio else "neutral")

        det_art = detalle_audit[detalle_audit["Producto costeo"] == "ART empacado 42,5 kg"].copy()
        subt_excluidos = det_art[~det_art["Incluido"]].copy()
        if not subt_excluidos.empty:
            st.warning("ART tiene subtotales excluidos para evitar doble conteo. El caso crítico es la fila CIF dentro de C MO ART EMP.")
            st.dataframe(fmt_df(subt_excluidos[["Bloque", "Produccion", "Indice", "Observacion", "Valor", "Motivo"]]), use_container_width=True, hide_index=True)
        else:
            st.success("No se detectaron subtotales excluidos en ART para este periodo.")

    section("Consolidado por índice")
    if not consolidado_audit.empty:
        st.dataframe(fmt_df(consolidado_audit), use_container_width=True, hide_index=True)
    else:
        st.info("No hay líneas de costeo para consolidar.")

    section("Detalle línea por línea")
    producto_sel = st.multiselect(
        "Filtrar producto",
        sorted(detalle_audit["Producto costeo"].dropna().unique().tolist()) if not detalle_audit.empty else [],
        default=["ART empacado 42,5 kg"] if not detalle_audit.empty and "ART empacado 42,5 kg" in detalle_audit["Producto costeo"].unique() else None,
    )
    detalle_view = detalle_audit.copy()
    if producto_sel:
        detalle_view = detalle_view[detalle_view["Producto costeo"].isin(producto_sel)].copy()
    cols_det = ["Producto costeo", "Bloque", "Produccion", "Indice", "Observacion", "Valor", "Incluido", "Valor incluido", "Motivo"]
    if not detalle_view.empty:
        st.dataframe(fmt_df(detalle_view[cols_det].sort_values(["Producto costeo", "Bloque", "Indice", "Incluido"])), use_container_width=True, hide_index=True)
    else:
        st.info("Sin detalle para el filtro seleccionado.")

    st.markdown(
        "<div class='decision-box'><b>Regla de control:</b> si una fila aparece como subtotal descriptivo y no como cuenta contable real, no debe entrar dos veces. "
        "El costeo correcto no es sumar todo lo que tenga un índice parecido; es sumar líneas contables limpias y auditar los subtotales.</div>",
        unsafe_allow_html=True,
    )
