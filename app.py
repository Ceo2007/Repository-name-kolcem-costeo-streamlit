    ...
    if costeo_df.empty:
        ...
        if componentes_costeo_df.empty:
            ...
            componentes_orden = [
                ...
                "Administración", "Ventas", "Financieros recurrentes", "Impuestos base",
            ...
            st.markdown("#### Vista ejecutiva compacta · costo, precio y margen por producto")
            st.caption(
                "Tabla diseñada para comité: muestra en una sola línea por producto cuánto cuesta producir, cuánto cuesta vender, "
                "a qué precio se vendió y qué utilidad dejó. Menos ruido, más decisión."
            )
            matriz_ejecutiva = matriz_bolsa.merge(
                costeo_df[[
                    "Producto", "Toneladas", "Toneladas vendidas", "Precio / bolsa", "Precio / ton",
                    "Utilidad / bolsa", "Utilidad / ton", "Margen", "Margen final contable", "Venta total", "Costo ventas real",
                    "Utilidad empresa", "Diferencia cambio no realizada", "Utilidad final contable", "Costo inventario no vendido", "Estado",
                ]],
                on="Producto",
                how="left",
            )
            matriz_ejecutiva["Costo industrial / bolsa"] = matriz_ejecutiva[["MP empacado", "MO empacado", "CIF empacado"]].sum(axis=1)
            matriz_ejecutiva["Gastos comerciales / bolsa"] = matriz_ejecutiva[["Administración", "Ventas", "Financieros recurrentes"]].sum(axis=1)
            matriz_ejecutiva["Impuestos / bolsa"] = matriz_ejecutiva[["Impuestos base", "Impuesto renta", "Impuesto patrimonio"]].sum(axis=1)
            matriz_ejecutiva["Brecha precio-costo / bolsa"] = matriz_ejecutiva["Precio / bolsa"] - matriz_ejecutiva["Costo total / bolsa"]
            matriz_ejecutiva["Lectura"] = np.select(
                [
                    pd.to_numeric(matriz_ejecutiva["Precio / bolsa"], errors="coerce").fillna(0) <= 0,
                    pd.to_numeric(matriz_ejecutiva["Utilidad empresa"], errors="coerce").fillna(0) < 0,
                    pd.to_numeric(matriz_ejecutiva["Margen"], errors="coerce").fillna(0) < 0.05,
                ],
                ["Sin venta", "Destruye valor", "Margen frágil"],
                default="Rentable",
            )
            cols_ejecutiva = [
                "Producto", "Kg bolsa", "Toneladas", "Toneladas vendidas",
                "MP empacado", "MO empacado", "CIF empacado", "Costo industrial / bolsa",
                "Administración", "Ventas", "Financieros recurrentes", "Impuestos / bolsa", "Gastos comerciales / bolsa",
                "Costo total / bolsa", "Precio / bolsa", "Brecha precio-costo / bolsa", "Utilidad / bolsa", "Margen",
                "Venta total", "Costo ventas real", "Utilidad empresa", "Diferencia cambio no realizada", "Utilidad final contable", "Margen final contable", "Costo inventario no vendido", "Lectura", "Estado",
            ]
            dataframe_gerencial(matriz_ejecutiva[[c for c in cols_ejecutiva if c in matriz_ejecutiva.columns]])

            st.markdown("#### Costo unitario técnico · $/bolsa y $/kg")
            dataframe_gerencial(matriz_bolsa[[c for c in cols_matriz_bolsa if c in matriz_bolsa.columns]])

            st.markdown("#### Detalle completo auditable por componente")
            detalle_cols = [
        ...
        dif_cambio_no_realizada_total = float(costeo_df.get("Diferencia cambio no realizada", pd.Series(dtype=float)).sum())
        utilidad_final_contable_total = float(costeo_df.get("Utilidad final contable", pd.Series(dtype=float)).sum())
        ...
        margen_final_contable_total = safe_div(utilidad_final_contable_total, venta_real_total)
        ...
        fxp1, fxp2, fxp3, fxp4 = st.columns(4)
        with fxp1:
            kpi("FX no realizado", money(dif_cambio_no_realizada_total), help_text="Separado del costo recurrente", tone="yellow" if abs(dif_cambio_no_realizada_total) > 0 else "neutral")
        with fxp2:
            kpi("Utilidad final contable", money(utilidad_final_contable_total), help_text=f"Margen: {pct(margen_final_contable_total)}", tone="red" if utilidad_final_contable_total < 0 else "green")
        with fxp3:
            kpi("Margen final contable", pct(margen_final_contable_total), tone="red" if margen_final_contable_total < 0 else "green")
        with fxp4:
            kpi("Criterio FX", "No precio base", help_text="La no realizada no manda tarifa", tone="neutral")
                        ...
                        <div class="kpi-card kpi-{tone}" style="min-height:340px;">
                          <div>
                            ...
                            <div style="display:grid;grid-template-columns:1.2fr 1fr;gap:7px 16px;color:var(--text-sec);font-size:.82rem;">
                              ...
                              <div>FX no realizado</div><div style="text-align:right;font-weight:800;">{fmt_money(row.get('Diferencia cambio no realizada',0))}</div>
                              <div>Utilidad final contable</div><div style="text-align:right;font-weight:800;">{fmt_money(row.get('Utilidad final contable',0))}</div>
        ...
        cols_costeo = [
            ...
            "Utilidad empresa", "Diferencia cambio realizada", "Diferencia cambio no realizada", "Utilidad final contable", "Utilidad / bolsa", "Utilidad / ton", "Margen", "Margen final contable",
        ...
        eco_df = costeo_df.melt(
            id_vars=["Nombre corto"],
            value_vars=["Venta total", "Costo ventas real", "Utilidad empresa", "Utilidad final contable", "Costo inventario no vendido"],
            var_name="Métrica",
            value_name="Valor",
        )
        fig = px.bar(
