...
# =============================================================================
# Simulación de producción, costeo y utilidad
# =============================================================================

def componentes_por_indices(df_mes: pd.DataFrame, produccion: str, indices: Iterable[str]) -> dict:
    """Separa el costo industrial en MP, MO, CIF y otros con base en los índices.

    La simulación usa esta apertura para proyectar costos con comportamientos distintos:
    MP suele ser variable, MO suele ser más fija y CIF puede ser mixto.
    """
    comps = {"MP": 0.0, "MO": 0.0, "CIF": 0.0, "OTROS": 0.0}
    for idx in indices:
        valor = sum_indices(df_mes, produccion, [idx])
        n = norm_text(idx)
        if " MP " in f" {n} " or n.startswith("C MP") or n.startswith("QI MP"):
            comps["MP"] += valor
        elif " MO " in f" {n} " or n.startswith("C MO"):
            comps["MO"] += valor
        elif " CIF " in f" {n} " or n.startswith("C CIF"):
            comps["CIF"] += valor
        else:
            comps["OTROS"] += valor
    return comps
...
def proyectar_componente(valor_base: float, factor_volumen: float, pct_variable: float) -> float:
    """Proyecta un componente separando la parte fija y la parte variable.

    Fórmula: costo nuevo = fijo base + variable base * factor.
    pct_variable debe estar entre 0 y 1.
    """
    pct_variable = min(max(float(pct_variable), 0.0), 1.0)
    if abs(valor_base) < 1e-12:
        return 0.0
    return valor_base * (1 - pct_variable) + valor_base * pct_variable * factor_volumen


def simular_costo_producto(
    costo_mp: float,
    costo_mo: float,
    costo_cif: float,
    costo_otros: float,
    ...
    pct_var_mo: float,
    pct_var_cif: float,
    pct_var_gastos: float,
) -> dict:
    mp = proyectar_componente(costo_mp, factor_volumen, pct_var_mp)
    mo = proyectar_componente(costo_mo, factor_volumen, pct_var_mo)
    cif = proyectar_componente(costo_cif, factor_volumen, pct_var_cif)
    otros = proyectar_componente(costo_otros, factor_volumen, pct_var_cif)
    gastos = proyectar_componente(gastos_base, factor_volumen, pct_var_gastos)
    total = mp + mo + cif + otros + gastos
    return {"MP simulada": mp, "MO simulada": mo, "CIF simulada": cif, "Otros simulados": otros, "Gastos simulados": gastos, "Costo total simulado": total}
...
with tabs[5]:
    section("Simulación de producción, nuevo costeo y utilidad")
    st.markdown(
        "<div class='executive-strip'><b>Cómo leer esta pestaña:</b> primero se recalcula el costo con las cantidades producidas simuladas. "
        "Después se calcula utilidad con las bolsas vendidas simuladas y el precio simulado. "
        "Materia prima se trata como variable por defecto; mano de obra como fija; CIF como mixto; gastos comerciales como fijos.</div>",
        unsafe_allow_html=True,
    )

    with st.expander("Supuestos de comportamiento del costo", expanded=True):
        s1, s2, s3, s4, s5 = st.columns(5)
        with s1:
            pct_var_mp = st.slider("MP variable", 0.0, 1.0, 1.0, 0.05, format="%.2f", key="sim_var_mp")
        with s2:
            pct_var_mo = st.slider("MO variable", 0.0, 1.0, 0.0, 0.05, format="%.2f", key="sim_var_mo")
        with s3:
            pct_var_cif = st.slider("CIF variable", 0.0, 1.0, 0.50, 0.05, format="%.2f", key="sim_var_cif")
        with s4:
            pct_var_gastos = st.slider("Gastos comerciales variables", 0.0, 1.0, 0.0, 0.05, format="%.2f", key="sim_var_gastos")
        with s5:
            incluir_extra_sim = st.checkbox("Restar extraordinarios actuales", value=False, key="sim_extra")
        st.markdown(
            "<div class='small-note'>Regla CFO: si subes producción, no todos los costos suben igual. "
            "MP normalmente sube con toneladas; MO y parte de CIF no necesariamente; gastos administrativos suelen permanecer fijos en el corto plazo.</div>",
            unsafe_allow_html=True,
        )

    section("Cantidades producidas, ventas y precios simulados")
    sim_inputs = {}
    for _, r in metrics.iterrows():
        prod_name = str(r["Producto"])
        with st.expander(f"{prod_name} · escenario", expanded=True):
            c1, c2, c3, c4 = st.columns(4)
            current_prod = float(r["Bolsas producidas"])
            current_sold = float(r["Bolsas vendidas"])
            current_price = float(r["Precio bolsa"])
            default_sold = current_prod if current_prod > 0 else current_sold
            with c1:
                sim_prod = st.number_input(
                    f"Bolsas producidas simuladas · {prod_name}",
                    min_value=0.0,
                    value=float(round(current_prod, 0)),
                    step=100.0,
                    format="%.0f",
                    key=f"sim_prod_{prod_name}",
                )
            with c2:
                sim_sold = st.number_input(
                    f"Bolsas vendidas simuladas · {prod_name}",
                    min_value=0.0,
                    value=float(round(default_sold, 0)),
                    step=100.0,
                    format="%.0f",
                    key=f"sim_sold_{prod_name}",
                )
            with c3:
                sim_price = st.number_input(
                    f"Precio/bolsa simulado · {prod_name}",
                    min_value=0.0,
                    value=float(round(current_price, 2)),
                    step=100.0,
                    format="%.2f",
                    key=f"sim_price_{prod_name}",
                )
            with c4:
                card("Base actual", f"{num(current_prod)} prod.", f"{num(current_sold)} vend. · {money(current_price)}", "neutral")
            sim_inputs[prod_name] = {"prod": sim_prod, "sold": sim_sold, "price": sim_price}

    section("Resultado simulado por producto vendible")
    sim_rows = []
    for _, r in metrics.iterrows():
        prod_name = str(r["Producto"])
        producto = VENDIBLES["UG_50"] if prod_name.startswith("UG") else VENDIBLES["ART_42_5"]
        inp = sim_inputs[prod_name]
        base_prod = float(r["Bolsas producidas"])
        sim_prod = float(inp["prod"])
        sim_sold = float(inp["sold"])
        sim_price = float(inp["price"])
        peso = float(r["Peso kg"])
        factor = safe_div(sim_prod, base_prod) if base_prod > 0 else 1.0
        comps = componentes_por_indices(df_mes, producto.prod_empacado, producto.indices_empacado)
        gastos_base = float(r["Gastos asignados"])
        sim_costs = simular_costo_producto(
            comps["MP"], comps["MO"], comps["CIF"], comps["OTROS"], gastos_base,
            factor, pct_var_mp, pct_var_mo, pct_var_cif, pct_var_gastos,
        )
        kg_sim = sim_prod * peso
        costo_bolsa_sim = safe_div(sim_costs["Costo total simulado"], sim_prod)
        costo_kg_sim = safe_div(sim_costs["Costo total simulado"], kg_sim)
        costo_vendido_sim = costo_bolsa_sim * sim_sold
        venta_sim = sim_price * sim_sold
        utilidad_sim = venta_sim - costo_vendido_sim
        margen_sim = safe_div(utilidad_sim, venta_sim) if venta_sim > 0 else np.nan
        inventario_sim = max(sim_prod - sim_sold, 0)
        sim_rows.append({
            "Producto": prod_name,
            "Bolsas producidas simuladas": sim_prod,
            "Kg producidos simulados": kg_sim,
            "Ton producidas simuladas": kg_sim / 1000,
            "Bolsas vendidas simuladas": sim_sold,
            "Precio simulado / bolsa": sim_price,
            "Costo / kg simulado": costo_kg_sim,
            "Costo / ton simulado": costo_kg_sim * 1000,
            "Costo / bolsa simulado": costo_bolsa_sim,
            "Costo total simulado": sim_costs["Costo total simulado"],
            "Costo vendido simulado": costo_vendido_sim,
            "Venta simulada": venta_sim,
            "Utilidad simulada": utilidad_sim,
            "Margen simulado": margen_sim,
            "Inventario bolsas simuladas": inventario_sim,
            "Costo inventario simulado": inventario_sim * costo_bolsa_sim,
            "MP simulada": sim_costs["MP simulada"],
            "MO simulada": sim_costs["MO simulada"],
            "CIF simulada": sim_costs["CIF simulada"],
            "Gastos simulados": sim_costs["Gastos simulados"],
        })

    sim_df = pd.DataFrame(sim_rows)
    extra_sim = float(extras["Valor"].sum()) if incluir_extra_sim and extras is not None and not extras.empty else 0.0
    venta_total_sim = float(sim_df["Venta simulada"].sum()) if not sim_df.empty else 0.0
    costo_vendido_total_sim = float(sim_df["Costo vendido simulado"].sum()) if not sim_df.empty else 0.0
    utilidad_antes_extra_sim = venta_total_sim - costo_vendido_total_sim
    utilidad_final_sim = utilidad_antes_extra_sim - extra_sim
    margen_final_sim = safe_div(utilidad_final_sim, venta_total_sim) if venta_total_sim > 0 else np.nan

    a1, a2, a3, a4, a5 = st.columns(5)
    with a1:
        card("Venta simulada", money(venta_total_sim), "Bolsas vendidas × precio", "blue")
    with a2:
        card("Costo vendido simulado", money(costo_vendido_total_sim), "Nuevo costo × ventas", "neutral")
    with a3:
        card("Utilidad antes de extra", money(utilidad_antes_extra_sim), "Resultado operativo simulado", "good" if utilidad_antes_extra_sim >= 0 else "bad")
    with a4:
        card("Extraordinarios simulados", money(extra_sim), "Opcional", "warn" if extra_sim else "neutral")
    with a5:
        card("Utilidad final simulada", money(utilidad_final_sim), f"Margen {pct(margen_final_sim)}", "good" if utilidad_final_sim >= 0 else "bad")

    st.dataframe(
        fmt_df(sim_df[[
            "Producto", "Costo / kg simulado", "Costo / ton simulado", "Costo / bolsa simulado",
            "Bolsas producidas simuladas", "Bolsas vendidas simuladas", "Venta simulada",
            "Costo vendido simulado", "Utilidad simulada", "Margen simulado",
    ...
    section("Apertura del costo simulado")
    st.dataframe(
        fmt_df(sim_df[[
            "Producto", "MP simulada", "MO simulada", "CIF simulada", "Gastos simulados", "Costo total simulado",
            "Costo / kg simulado", "Costo / bolsa simulado",
        ]]),
        use_container_width=True,
        hide_index=True,
    )

    section("Costeo físico simulado de granel")
    granel_rows = []
    for cfg in [x for x in FISICOS if x["Tipo"] == "Granel"]:
        base_kg = sum_obs(df_mes, cfg["Produccion"], cfg["Obs cantidad"])
        sim_kg = st.number_input(
            f"Kg producidos simulados · {cfg['Producto']}",
            min_value=0.0,
            value=float(round(base_kg, 0)),
            step=1000.0,
            format="%.0f",
            key=f"sim_granel_{cfg['Producto']}",
        )
        factor = safe_div(sim_kg, base_kg) if base_kg > 0 else 1.0
        comps = componentes_por_indices(df_mes, cfg["Produccion"], cfg["Indices costo"])
        sim_costs = simular_costo_producto(
            comps["MP"], comps["MO"], comps["CIF"], comps["OTROS"], 0.0,
            factor, pct_var_mp, pct_var_mo, pct_var_cif, 0.0,
        )
        granel_rows.append({
            "Producto": cfg["Producto"],
            "Kg producidos simulados": sim_kg,
            "Ton producidas simuladas": sim_kg / 1000,
            "Costo / kg simulado": safe_div(sim_costs["Costo total simulado"], sim_kg),
            "Costo / ton simulado": safe_div(sim_costs["Costo total simulado"], sim_kg) * 1000,
            "Costo total simulado": sim_costs["Costo total simulado"],
            "MP simulada": sim_costs["MP simulada"],
            "MO simulada": sim_costs["MO simulada"],
            "CIF simulada": sim_costs["CIF simulada"],
        })
    granel_sim_df = pd.DataFrame(granel_rows)
    st.dataframe(fmt_df(granel_sim_df), use_container_width=True, hide_index=True)

    section("Advertencias de simulación")
    sim_warn = [
        "La utilidad simulada no es P&G oficial: es una proyección gerencial con los supuestos de costo elegidos.",
        "Si producción simulada sube mucho, valide capacidad, mantenimiento, inventario y ventas antes de creer el margen.",
        "Si no cambia bolsas vendidas, producir más mejora inventario, no necesariamente utilidad.",
    ]
    if (sim_df["Bolsas vendidas simuladas"] > sim_df["Bolsas producidas simuladas"]).any():
        sim_warn.append("Hay productos con ventas simuladas superiores a producción simulada; eso exige inventario inicial disponible.")
    st.dataframe(pd.DataFrame({"Advertencia": sim_warn}), use_container_width=True, hide_index=True)
